...
struct TransferCallback
{
    struct Control
    {
        static inline auto disable = py::none();
        static inline auto keep = py::cast<py::function>(py::none());
    };
    ...
    TransferCallback() : cb_interval_s(0.5f) {}

    TransferCallback(float cb_interval_s, py::function error_cb = py::function(), py::function progress_cb = py::function(), py::function filename_cb = py::function())
    ...
    {
        auto process_cb = [](py::object input, py::function &output)
        {
            if (input.is(Control::disable))
            {
                output = py::function(); // 显式禁用
            }
            else if (!input.is(Control::keep))
            {
                if (!input.is_none())
                {
                    output = input.cast<py::function>();
                }
            }
            // 如果是keep则保持output的默认空状态
        };

        process_cb(error, error_cb);
        process_cb(progress, progress_cb);
        process_cb(filename, filename_cb);
    }

    // 动态更新方法
    void update(py::str type, py::object cb)
    {
        auto &target = [&]() -> py::function &
        {
            if (type.equal(py::str("error")))
                return error_cb;
            if (type.equal(py::str("progress")))
                return progress_cb;
            return filename_cb;
        }();

        if (cb.is(Control::disable))
        {
            target = py::function();
        }
        else if (!cb.is(Control::keep))
        {
            if (!cb.is_none())
            {
                target = cb.cast<py::function>();
            }
        }
    }

    bool has_error_cb() const { return !error_cb.is_none(); }
    bool has_progress_cb() const { return !progress_cb.is_none(); }
    bool has_filename_cb() const { return !filename_cb.is_none(); }
};
...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        if (!dstFile)
        {
            ...
            if (rct == LIBSSH2_FX_NO_SUCH_FILE)
            {
                libssh2_sftp_mkdir(another_worker->amsession->sftp, dirname(dst).c_str(), 0744);
                dstFile = libssh2_sftp_open(another_worker->amsession->sftp, dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC, 0744);
                if (!dstFile)
                {
                    return EC::RemoteFileOpenError;
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<TransferCallback>(m, "TransferCallback")
        .def(py::init<float, py::object, py::object, py::object>(), py::arg("cb_interval_s") = 0.5, py::arg("error_cb") = py::none(), py::arg("progress_cb") = py::none(), py::arg("filename_cb") = py::none())
