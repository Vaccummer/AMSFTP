...
private:
    std::vector<T> buffer;
    size_t capacity;
    size_t head = 0;  // 当前头部索引（最旧元素）
    size_t tail = 0;  // 当前尾部索引（下一个写入位置）
...
public:
    ...
    void print() const
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            std::cout << buffer[current].first << ": " << buffer[current].second << std::endl;
            current = (current + 1) % capacity;
        }
        std::cout << "\n";
    }
...
public:
    ...
    EC check()
    {
        ...
        {
            rc = EC::SessionBroken;
            error_info = {rc, "Session is not initialized"};
            return rc;
        }
        else
        {
