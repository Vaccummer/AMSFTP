...
private:
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        while (offset < file_size)
        {
            ...
            std::cout << "rc_read: " << rc_read / AMKB << "," << "chunk_size: " << chunk_size / AMKB << std::endl;
            if (is_terminate.load())
            {
            ...
            if (rc_read > 0)
            {
                ...
                while (local_write < rc_read)
                {
                    ...
                    if (rc_write > 0)
                    {
                        ...
                        if (count % adjust_interval == 0)
                        {
                            speed = static_cast<double>(rc_write) / (time_end - time_start > 0 ? time_end - time_start : 1E-7);
                            buffer_size = calculate_buffer_size(file_size - offset, speed);
                        }
                        count++;
