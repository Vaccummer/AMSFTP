...
{
    std::unordered_map<std::string, SingleBuffer> bufferd;
    TransferContext(size_t buffer_size)
    {
        bufferd["a"] = SingleBuffer(buffer_size);
        bufferd["b"] = SingleBuffer(buffer_size);
    }

    std::string get_write_buffer()
    {
        if (bufferd["a"].status == BufferStatus::is_writing)
        {
            return "a";
        }
        else if (bufferd["b"].status == BufferStatus::is_writing)
        {
            return "b";
        }
        else if (bufferd["a"].status == BufferStatus::read_done)
        {
            bufferd["a"].status = BufferStatus::is_writing;
            return "a";
        }
        else if (bufferd["b"].status == BufferStatus::read_done)
        {
            bufferd["b"].status = BufferStatus::is_writing;
            return "b";
        }
        else
        {
    ...
    {
        if (bufferd["a"].status == BufferStatus::is_reading)
        {
            return "a";
        }
        else if (bufferd["b"].status == BufferStatus::is_reading)
        {
            return "b";
        }
        else if (bufferd["a"].status == BufferStatus::write_done)
        {
            bufferd["a"].status = BufferStatus::is_reading;
            return "a";
        }
        else if (bufferd["b"].status == BufferStatus::write_done)
        {
            bufferd["b"].status = BufferStatus::is_reading;
            return "b";
        }
        else
        {
    ...
    void finish_write(std::string buffer_name)
    {
        bufferd[buffer_name].status = BufferStatus::write_done;
        bufferd[buffer_name].written = 0;
    }

    void finish_read(std::string buffer_name)
    {
...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        while (all_write < file_size)
        {
            ...
            if (!rbuffer.empty() && all_read < file_size)
            {
                do
                {
                    ...
                    if (rc_read > 0)
                    {
                        ...
                        {
                            ctx.bufferd[rbuffer].status = BufferStatus::read_done;
                            ctx.bufferd[rbuffer].read = 0;
            ...
            if (wbuffer != "" && all_write < file_size)
            {
                do
                {
                    // libssh2_sftp_seek64(dstFile, all_write);
                    rc_write = libssh2_sftp_write(dstFile, ctx.bufferd[wbuffer].bufferptr + ctx.bufferd[wbuffer].written, wchunk_size - ctx.bufferd[wbuffer].written);
                    if (rc_write > 0)
                    {
                        all_write += rc_write;
                        ctx.bufferd[wbuffer].written += rc_write;
                        if (ctx.bufferd[wbuffer].written == wchunk_size)
                        {
                            ctx.bufferd[wbuffer].status = 0;
                            ctx.bufferd[wbuffer].written = 0;
