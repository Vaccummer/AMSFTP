...
public:
    ...
    EC saferm(std::string path)
    {
        ...
        if (rc != EC::PassCheck)
        {
            ...
            if (rc != EC::Success)
            {
                error_info_buffer.push(ErrorInfo(rc, path, "", "AMSFTPClient::saferm", "Trash dir mkdir failed"));
                return rc;
            }
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push(ErrorInfo(rc, path, "", "AMSFTPClient::saferm", "Rename failed"));
            return rc;
        }
        return EC::Success;
    }
    ...
    EC move(std::string src, std::string dst, bool need_mkdir = false, bool force_write = false)
    {
        ...
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push(ErrorInfo(rc, src, "", "AMSFTPClient::move", "Src not exists"));
            return rc;
        }
