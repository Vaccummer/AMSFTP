...
{
public:
    AMPath(const fs::path &path)
    {
        m_path = normalize_separators(path.string());
    }

    AMPath(const std::string &str) : AMPath(fs::path(str)) {}

    AMPath(const char *str) : AMPath(fs::path(str)) {}

    operator fs::path() const { return fs::path(m_path); }

    AMPath operator+(const AMPath &rhs) const
    {
        return AMPath((fs::path(m_path) / fs::path(rhs.m_path)).string());
    }
    ...
    WindowsPath dirname() const
    {
        return WindowsPath(fs::path(m_path).parent_path());
    }
...
public:
    ...
    LR listdir(std::string path)
    {
        ...
        while (true)
        {
            ...
            {
                name.assign(safe_filename_buffer->data(), rc); // 使用实际读取的长度
                // 确保字符串以 null 结尾（可选）
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<AMSFTPClient>(m, "AMSFTPClient")
        ...
        .def("stat", &AMSFTPClient::stat, py::arg("path"), py::arg("need_reconnect") = false)
        ...
        .def("listdir", &AMSFTPClient::listdir, py::arg("path"), py::arg("need_reconnect") = false)
        .def("mkdir", &AMSFTPClient::mkdir, py::arg("path"), py::arg("need_reconnect") = false)
        .def("mkdirs", &AMSFTPClient::mkdirs, py::arg("path"), py::arg("need_reconnect") = false)
        .def("rmfile", &AMSFTPClient::rmfile, py::arg("path"), py::arg("need_reconnect") = false)
        .def("rmdir", &AMSFTPClient::rmdir, py::arg("path"), py::arg("need_reconnect") = false)
        .def("rm", &AMSFTPClient::rm, py::arg("path"))
        .def("saferm", &AMSFTPClient::saferm, py::arg("path"), py::arg("need_reconnect") = false)
