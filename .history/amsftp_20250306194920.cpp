...
struct ConRequst
{
    ...
    ConRequst()
        : hostname(""), username(""), password(""), port(22), compression(false), timeout_ms(3000), trash_dir("") {}
    ConRequst(std::string hostname, std::string username, std::string password, int port, bool compression, size_t timeout_ms = 3000, std::string trash_dir = "")
        : hostname(hostname), username(username), password(password), port(port), compression(compression), timeout_ms(timeout_ms), trash_dir(trash_dir) {}
};
...
public:
    ...
    EC init()
    {
        ...
        while (libssh2_session_handshake(session, sock) == LIBSSH2_ERROR_EAGAIN)
        {
            ...
            double cb_time_now = std::chrono::duration<double>(
                                     std::chrono::system_clock::now().time_since_epoch())
                                     .count();
            if ((cb_time_now - cb_time) * 1000 > request.timeout_ms)
            {
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<ConRequst>(m, "ConRequst")
        .def(py::init<std::string, std::string, std::string, int, bool, size_t, std::string>(), py::arg("hostname"), py::arg("username"), py::arg("password"), py::arg("port"), py::arg("compression"), py::arg("timeout_ms") = 3000, py::arg("trash_dir") = "")
        ...
        .def_readwrite("timeout_ms", &ConRequst::timeout_ms)
