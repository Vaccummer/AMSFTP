...
private:
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        const int max_fd = std::max(src_sock, dst_sock) + 1;

        TransferContext ctx(buffer_size_ori);
        char *read_buf = ctx.buf_a.get();
        size_t *read_size = &ctx.buf_a_size;
        ...
        while (!(ctx.read_complete && ctx.write_complete))
        {
            // 处理终止信号
            if (is_terminate.load())
            {
            ...
            // 设置监听事件
            if (!ctx.read_complete)
                FD_SET(src_sock, &read_fds);
            if (write_buf)
                FD_SET(dst_sock, &write_fds);

            int ready = select(max_fd, &read_fds, &write_fds, nullptr, &timeout);
            if (ready < 0)
                break;

            // === 数据读取阶段 ===
        if (FD_ISSET(src_sock, &read_fds) {
                ssize_t rc = libssh2_sftp_read(srcFile, read_buf + *read_size,
                                               buffer_size_ori - *read_size);

                if (rc > 0)
                {
                    *read_size += rc;
                    // 缓冲区填满时切换
                    if (*read_size == buffer_size_ori)
                    {
                        if (read_buf == ctx.buf_a.get())
                        {
                            ctx.buf_a_ready = true;
                            read_buf = ctx.buf_b.get();
                            read_size = &ctx.buf_b_size;
                        }
                        else
                        {
                            ctx.buf_b_ready = true;
                            read_buf = ctx.buf_a.get();
                            read_size = &ctx.buf_a_size;
                        }
                    }
                }
                else if (rc == 0)
                ...
                {
                    rc_final = handle_error(src_session);
                    break;
                }
        }

        // === 数据写入阶段 ===
        if (FD_ISSET(dst_sock, &write_fds) && write_buf) {
                ssize_t rc = libssh2_sftp_write(dstFile,
                                                write_buf + (buffer_size_ori - *write_size),
                                                *write_size);

                if (rc > 0)
                {
                    ctx.total_written += rc;
                    *write_size -= rc;

                    // 当前缓冲区写入完成
                    if (*write_size == 0)
                    {
                        if (write_buf == ctx.buf_a.get())
                        {
                            ctx.buf_a_ready = false;
                            ctx.buf_a_size = 0;
                        }
                        else
                        {
                            ctx.buf_b_ready = false;
                            ctx.buf_b_size = 0;
                        }
                        write_buf = nullptr;
                    }
                    ...
                    // 更新进度回调
                    update_progress(ctx.total_written);
                }
                else if (rc != LIBSSH2_ERROR_EAGAIN)
                {
                    rc_final = handle_error(dst_session);
                    break;
                }
        }
        ...
        if (!write_buf) {
                if (ctx.buf_a_ready)
                {
                    write_buf = ctx.buf_a.get();
                    write_size = &ctx.buf_a_size;
                }
                else if (ctx.buf_b_ready)
                {
                    write_buf = ctx.buf_b.get();
                    write_size = &ctx.buf_b_size;
                }
        }

        // === 完成条件检查 ===
        if (ctx.read_complete && !ctx.buf_a_ready && !ctx.buf_b_ready) {
                ctx.write_complete = (ctx.total_written == file_size);
        }
        }
