...
constexpr wchar_t *AMLEVEL = L"LEVEL";
constexpr wchar_t *AMERRORNAME = L"ERRORNAME";
constexpr wchar_t *AMTARGET = L"TARGET";
constexpr wchar_t *AMACTION = L"ACTION";
constexpr wchar_t *AMMESSAGE = L"MESSAGE";

constexpr wchar_t *AMCRITICAL = L"CRITICAL";
constexpr wchar_t *AMERROR = L"ERROR";
constexpr wchar_t *AMWARNING = L"WARNING";
constexpr wchar_t *AMDEBUG = L"DEBUG";
constexpr wchar_t *AMINFO = L"INFO";

std::wstring sftp_error_code_cast(unsigned long errcode)
{
    switch (errcode)
    {
    case LIBSSH2_FX_OK:
    ...
    case LIBSSH2_FX_NO_SUCH_PATH:
        return L"No such path";
    case LIBSSH2_FX_FILE_ALREADY_EXISTS:
        return L"File already exists";
    case LIBSSH2_FX_WRITE_PROTECT:
        return L"Write protected filesystem";
    case LIBSSH2_FX_NO_MEDIA:
        return L"No media in drive";
    default:
        return L"Unknown error";
    }
}

std::vector<std::wstring> make_py_error(std::wstring errorname, std::wstring tar, std::wstring act)
{
...
void lmkdirs(const std::wstring &path)
{
    try
    {
...
struct ConRequst
{
    ...
    int port;
    size_t timeout_s;
    std::wstring trash_dir = L"";
    ConRequst()
        : nickname(L""), hostname(""), username(L""), password(L""), port(22), compression(false), timeout_s(3), trash_dir(L"") {}
    ConRequst(std::wstring nickname, std::string hostname, std::wstring username, std::wstring password, int port, bool compression, size_t timeout_s = 3, std::wstring trash_dir = L"")
...
struct PathInfo
{
    ...
    PathInfo()
        : name(L""), path(L""), dir(L""), size(-1), atime(-1), mtime(-1), path_type(PathType::FILE) {}
...
public:
    ...
    FileMapper(std::wstring &file_path, MapType map_type, std::wstring &error_msg, uint64_t file_size = 0)
    {
        LARGE_INTEGER li;
...
struct ErrorInfo
{
    ...
    ErrorInfo()
        : error_code(TransferErrorCode::UnknownError), src(L""), dst(L""), function_name(L""), msg(L"") {}
    ...
    {
        return L"ErrorInfo(error_code=" + std::to_wstring(static_cast<int>(error_code)) + L", src=" + src + L", dst=" + dst + L", function_name=" + function_name + L", msg=" + msg + L")";
    }
};
...
using TRM = std::vector<std::pair<std::pair<std::wstring, std::wstring>, TransferErrorCode>>;
...
void _walk(std::wstring path, WRV &result)
{
    ...
    case fs::file_type::directory:
        for (const auto &entry : fs::directory_iterator(p))
        {
            ...
            {
                subdirs.push_back(entry.path().wstring());
            }
            else
            {
...
public:
    ...
    EC init()
    {
        ...
        {
            pytrace(AMCRITICAL, L"SocketCreateError", request.nickname, L"CreateSocket", L"Create but get invalid socket");
        ...
        if (selectRet <= 0)
        {
            if (selectRet == 0)
            {
                pytrace(AMCRITICAL, L"SocketConnectError", request.nickname, L"ConnectSocket", L"Connect Timeout");
            }
            else
            {
                pytrace(AMCRITICAL, L"SocketConnectError", request.nickname, L"ConnectSocket", L"Connect Failed");
            }
        ...
        if (!session)
        {
            pytrace(AMCRITICAL, L"SessionCreateError", request.nickname, L"SessionInit", L"Session initialization failed");
        ...
        if (rc_handshake != 0)
        {
            pytrace(AMCRITICAL, L"HandshakeEstablishError", request.nickname, L"SessionHandshake", L"Session handshake failed");
        ...
        if (!request.password.empty())
        {
            ...
            if (rca == 0)
            {
                ...
                pytrace(AMINFO, L"AuthorizeSuccess", request.nickname, L"PasswordAuthorize", L"Password authorize success");
                goto OK;
            }
            else
            {
    ...
    OK:
        ...
        if (request.trash_dir.empty())
        {
            if (cwd.empty())
            {
                request.trash_dir = ".amsftp_trash";
            }
            else
            {
                request.trash_dir = join_path(cwd, ".amsftp_trash");
            }
            pytrace(AMINFO, "", "TrashDir", "DefaultTrashDir", fmt::format("Trash directory set to default: \"{}\"", request.trash_dir));
        }
    ...
    EC check()
    {
        ...
        if (rct != 0)
        {
            ...
            std::wstring msg = sftp_error_code_cast(libssh2_sftp_last_error(sftp));
    ...
    void pytrace(std::wstring level, std::wstring error_name = "", std::wstring target = "", std::wstring action = "", std::wstring msg = "")
    {
        if (is_trace.load(std::memory_order_acquire))
        {
            std::unordered_map<std::wstring, std::wstring> level_map = {
                {AMLEVEL, level},
...
private:
    ...
    void _get_size(std::wstring path, uint64_t &size, EC &error, bool exclude_dir = false, bool exclude_symlink = false)
    {
        SR info = stat(path);
...
public:
    ...
    std::wstring get_session_error_msg()
    {
        char *errmsg = NULL;
        int errmsg_len;
    ...
    // std::wstring get_sftp_error_msg()
    // {
    ...
    EC ensure_trash_dir()
    {
        ...
        if (rc != EC::Success)
        {
            std::wstring target = fmt::format("{}@{}", request.nickname, request.port);
    ...
    EC init(py::object pycb = py::none())
    {
        SetPyTrace(pycb);
    ...
    SIZER get_size(std::wstring path, bool exclude_dir = false, bool exclude_symlink = false)
    {
        uint64_t size = 0;
        EC error = EC::Success;
...
private:
    ...
    EC _legacy()
    {
        // EC _Local2Remote2(const std::wstring &src, const std::wstring &dst, uint64_t &chunk_size)
        // {
        //     TransferErrorCode rc_r = EC::Success;
        //     long rct;
        //     LIBSSH2_SFTP_HANDLE *sftpFile;
        //     std::wstring error_msg = "Error to create FileMapper";
        ...
        //     uint64_t buffer_size = std::min<uint64_t>(buffer_size, remaining);
        ...
        // EC _Remote2Remote_ori(const std::wstring &src, const std::wstring &dst, std::shared_ptr<AMSFTPWorker> another_worker)
        // {
        ...
        //     uint64_t buffer_size_ori = 256 * AMKB;
        ...
        // EC _Remote2Remote2(const std::wstring &src, const std::wstring &dst, std::shared_ptr<AMSFTPWorker> another_worker)
        // {
        //     TransferErrorCode rc_final = EC::Success;
        //     long rct;
        //     LIBSSH2_SFTP_HANDLE *srcFile;
        ...
        //                 rchunk_size = std::min<uint64_t>(512 * AMKB, file_size - all_read);
        //             }
        //         }
    ...
    EC Local2Remote(const std::wstring &src, const std::wstring &dst, uint64_t &chunk_size)
    {
        TransferErrorCode rc_r = EC::Success;
        long rct;
        LIBSSH2_SFTP_HANDLE *sftpFile;
    ...
    EC Remote2Local(const std::wstring &src, const std::wstring &dst, uint64_t &chunk_size)
    {
    ...
    EC Remote2Remote(const std::wstring &src, const std::wstring &dst, std::shared_ptr<AMSFTPWorker> another_worker, uint64_t &chunk_size)
    {
        TransferErrorCode rc_final = EC::Success;
        long rct;
        LIBSSH2_SFTP_HANDLE *srcFile;
            ...
            if (wbuffer != -1 && all_write < file_size)
            {
                ...
                if (ctx.bufferd[wbuffer].written == ctx.bufferd[wbuffer].read)
                {
                    ...
                    if (callback.need_progress_cb && time_end - cb_time > callback.cb_interval_s)
                    {
                        cb_time = time_end;
                        py::gil_scoped_acquire acquire;
                        callback.progress_cb(all_write, total_size);
                    }
                    ctx.finish_write(wbuffer, write_order++);
                    break;
                }
    ...
    clean:
        ...
        {
            py::gil_scoped_acquire acquire;
            callback.progress_cb(all_write, file_size);
        }
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<ConRequst>(m, "ConRequst")
        .def(py::init<std::wstring, std::wstring, std::wstring, std::wstring, int, bool, size_t, std::wstring>(), py::arg("nickname"), py::arg("hostname"), py::arg("username"), py::arg("password"), py::arg("port"), py::arg("compression"), py::arg("timeout_s") = 3, py::arg("trash_dir") = "")
    ...
    py::class_<TransferTask>(m, "TransferTask")
        .def(py::init<std::wstring, std::wstring, uint64_t>(), py::arg("src"), py::arg("dst"), py::arg("size"))
    ...
    py::class_<AMSFTPWorker, std::shared_ptr<AMSFTPWorker>>(m, "AMSFTPWorker")
        .def(py::init<std::vector<std::wstring>, ConRequst, uint64_t>(), py::arg("private_keys"), py::arg("request"), py::arg("error_info_buffer_size") = 10)
