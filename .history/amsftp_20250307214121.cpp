...
struct TransferContext
{
    ...
    TransferContext(uint64_t buffer_size)
    {
...
private:
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        while (ctx.offset < file_size || active_write_buf != nullptr)
        {
            ...
            {
                int rc_read = libssh2_sftp_read(srcFile, active_read_buf + read_offset,
            ...
            if (is_writing)
            {
                int rc_write = libssh2_sftp_write(dstFile, active_write_buf + write_offset,
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<AMSFTPWorker>(m, "AMSFTPWorker")
        .def(py::init<uint64_t, std::vector<std::string>, ConRequst, TransferCallback, TASKS, ConRequst>(), py::arg("ID"), py::arg("private_keys"), py::arg("request"), py::arg("callback"), py::arg("tasks"), py::arg("src_request") = ConRequst(), py::arg("dst_request") = ConRequst())
