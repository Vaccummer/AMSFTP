        ...
        while (ctx.offset < file_size || active_write_buf != nullptr)
        {
            ...
            if (is_writing)
            {
                int rc_write = libssh2_sftp_write(dstFile, active_write_buf + write_offset,
                                                  BUFFER_BASE_SIZE - write_offset);
                if (rc_write > 0)
                {
                    write_offset += rc_write;
                    if (write_offset == BUFFER_BASE_SIZE)
                    {
                        ctx.offset += BUFFER_BASE_SIZE;
                        is_writing = false;
                        active_write_buf = nullptr;
                        // 触发回调更新进度
                        current_size += BUFFER_BASE_SIZE;
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<AMSFTPWorker>(m, "AMSFTPWorker")
        .def(py::init<uint64_t, std::vector<std::string>, ConRequst, TransferCallback, TASKS, ConRequst>(), py::arg("ID"), py::arg("private_keys"), py::arg("request"), py::arg("callback"), py::arg("tasks"), py::arg("extra_request") = ConRequst())
