...
struct TransferCallback
{
    ...
    TransferCallback(py::object error_cb, py::object progress_cb, py::object filename_cb, float cb_interval_s)
        : progress_cb(progress_cb), filename_cb(filename_cb), error_cb(error_cb), cb_interval_s(cb_interval_s)
    {
        this->need_progress_cb = false;
        this->need_filename_cb = false;
        this->need_error_cb = false;
        if (py::isinstance<py::function>(progress_cb))
        {
            this->need_progress_cb = true;
        }
        if (py::isinstance<py::function>(filename_cb))
        {
            this->need_filename_cb = true;
        }
        if (py::isinstance<py::function>(error_cb))
        {
            this->need_error_cb = true;
        }
    }
};
...
public:
    ...
    TR upload(TASKS tasks, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        ...
        for (auto &task : tasks)
        {
            ...
            result[task.src] = Local2Remote(task.src, task.dst);
    ...
    TR download(TASKS tasks, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        ...
        for (auto &task : tasks)
        {
            ...
            result[task.src] = Remote2Local(task.src, task.dst);
            if (rc != EC::Success && callback.need_error_cb)
            {
                py::gil_scoped_acquire acquire;
                callback.error_cb(current_filename, rc);
    ...
    TR toAnotherHost(TASKS tasks, std::shared_ptr<AMSFTPClient> another_client, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        std::lock_guard<std::recursive_mutex> lock(transfer_mtx);
        reset();
        this->callback = cb_set;
        ...
        for (auto &task : tasks)
        {
            ...
            result[task.src] = Remote2Remote(task.src, task.dst, another_client);
            if (rc != EC::Success && callback.need_error_cb)
            {
                py::gil_scoped_acquire acquire;
                callback.error_cb(current_filename, rc);
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<TransferCallback>(m, "TransferCallback")
        .def(py::init<py::function, py::function, py::function, float>(), py::arg("error_cb"), py::arg("progress_cb"), py::arg("filename_cb"), py::arg("cb_interval_s"))
    ...
    py::class_<AMSFTPWorker>(m, "AMSFTPWorker")
        ...
        .def("upload", &AMSFTPWorker::upload, py::arg("tasks"), py::arg("need_mkdir") = false, py::arg("force_write") = false)
        .def("download", &AMSFTPWorker::download, py::arg("tasks"), py::arg("need_mkdir") = false, py::arg("force_write") = false)
        .def("toAnotherHost", &AMSFTPWorker::toAnotherHost, py::arg("tasks"), py::arg("another_client") = false, py::arg("cb_set") = TransferCallback(), py::arg("buffer_set") = BufferSet());
}
