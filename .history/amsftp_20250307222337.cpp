...
struct TransferContext2
{
    char *buffer_a = nullptr;
    char *buffer_b = nullptr;
    bool buffer_a_ready = false;
    bool buffer_b_ready = false;
    uint64_t offset = 0;
    bool reading_finished = false;
    TransferContext2(uint64_t buffer_size)
    {
        buffer_a = new char[buffer_size];
        buffer_b = new char[buffer_size];
    }
    ~TransferContext2()
    {
        delete[] buffer_a;
...
private:
    ...
    EC Remote2Remote2(const TransferTask &task)
    {
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        const int src_sock = libssh2_session_get_socket(src_session->session);
        const int dst_sock = libssh2_session_get_socket(dst_session->session);
        const int maxfd = std::max(src_sock, dst_sock) + 1;

        TransferContext ctx(buffer_size_ori, file_size);
        char *active_read_buf = ctx.buffer_a;
        char *active_write_buf = nullptr;
        size_t *active_read_size = &ctx.buffer_a_size;
        size_t *active_write_size = nullptr;
        bool read_eof = false;

        fd_set readfds, writefds;
        struct timeval timeout;

        while (!ctx.reading_finished || !ctx.writing_finished)
        {
            if (is_terminate.load())
            {
                rc_final = EC::Terminate;
            ...
            if (!ctx.reading_finished)
                FD_SET(src_sock, &readfds);
            if (active_write_buf)
                FD_SET(dst_sock, &writefds);

            int rc = select(maxfd, &readfds, &writefds, NULL, &timeout);
            if (rc < 0)
                break;

            // Reading phase
        if (FD_ISSET(src_sock, &readfds) {
                ssize_t rc_read = libssh2_sftp_read(srcFile,
                                                    active_read_buf + *active_read_size,
                                                    buffer_size_ori - *active_read_size);

                if (rc_read > 0)
                {
                    *active_read_size += rc_read;
                    if (*active_read_size == buffer_size_ori)
                    {
                        // Switch buffer
                        if (active_read_buf == ctx.buffer_a)
                        {
                            ctx.buffer_a_ready = true;
                            active_read_buf = ctx.buffer_b;
                            active_read_size = &ctx.buffer_b_size;
                        }
                        else
                        {
                            ctx.buffer_b_ready = true;
                            active_read_buf = ctx.buffer_a;
                            active_read_size = &ctx.buffer_a_size;
                        }
                    }
                }
                else if (rc_read == 0)
                {
                    ctx.reading_finished = true;
                }
        ...
        // Writing phase
        if (FD_ISSET(dst_sock, &writefds) && active_write_buf) {
                ssize_t rc_write = libssh2_sftp_write(dstFile,
                                                      active_write_buf + (buffer_size_ori - *active_write_size),
                                                      *active_write_size);

                if (rc_write > 0)
                {
                    ctx.total_written += rc_write;
                    *active_write_size -= rc_write;

                    if (*active_write_size == 0)
                    {
                        // Switch buffer
                        if (active_write_buf == ctx.buffer_a)
                        {
                            ctx.buffer_a_ready = false;
                            ctx.buffer_a_size = 0;
                        }
                        else
                        {
                            ctx.buffer_b_ready = false;
                            ctx.buffer_b_size = 0;
                        }
                        active_write_buf = nullptr;
                    }

                    // Update progress
                    if (callback.need_progress_cb)
                    {
                        double current_time = /* get timestamp */;
                        if (current_time - cb_time > callback.cb_interval_s)
                        {
                            py::gil_scoped_acquire acquire;
                            callback.progress_cb(ctx.total_written, callback.total_bytes);
                            cb_time = current_time;
                        }
                    }
                }
        ...
        // Activate next write buffer
        if (!active_write_buf) {
                if (ctx.buffer_a_ready)
                {
                    active_write_buf = ctx.buffer_a;
                    active_write_size = &ctx.buffer_a_size;
                }
                else if (ctx.buffer_b_ready)
                {
                    active_write_buf = ctx.buffer_b;
                    active_write_size = &ctx.buffer_b_size;
                }
        }

        // Final completion check
        if (ctx.reading_finished && !ctx.buffer_a_ready && !ctx.buffer_b_ready) {
                ctx.writing_finished = (ctx.total_written == ctx.file_size);
        }
        }
