...
struct ConRequst
{
    ...
    ConRequst()
        : hostname(""), username(""), password(""), port(22), compression(false), trash_dir("") {}
    ConRequst(std::string hostname, std::string username, std::string password, int port, bool compression, std::string trash_dir = "")
        : hostname(hostname), username(username), password(password), port(port), compression(compression), trash_dir(trash_dir) {}
};
...
struct TransferCallback
{
    ...
    TransferCallback(py::function error_cb, py::function progress_cb, py::function filename_cb, float cb_interval_ms, uint64_t total_bytes, bool need_error_cb, bool need_progress_cb, bool need_filename_cb)
        : progress_cb(progress_cb), filename_cb(filename_cb), error_cb(error_cb), cb_interval_s(cb_interval_ms), total_bytes(total_bytes), need_progress_cb(need_progress_cb), need_filename_cb(need_filename_cb), need_error_cb(need_error_cb) {}
};
...
private:
    ...
    uint64_t
    calculate_buffer_size(uint64_t file_size, uint64_t speed)
    {
    ...
    EC Local2Remote(const TransferTask &task)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (rc > 0)
            {
                ...
                if (speed_interval_cout % adjust_interval == 0)
                {
                    speed = static_cast<double>(chunk_size / (time_end - time_start > 0 ? time_end - time_start : 0.01));
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        double speed = 10 * AMGB;
        uint64_t buffer_size_ori = calculate_buffer_size(file_size, 9999999999);
        uint64_t buffer_size = buffer_size_ori;
        char *buffer = new char[buffer_size_ori];
        ...
        while (offset < file_size)
        {
            ...
            if (rc_read > 0)
            {
                ...
                if (rc_write > 0)
                {
                    ...
                    if (count % adjust_interval == 0)
                    {
                        speed = static_cast<double>(rc_write) / (time_end - time_start > 0 ? time_end - time_start : 0.01);
                        buffer_size = calculate_buffer_size(file_size - offset, speed);
                    }
                    count++;
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<ConRequst>(m, "ConRequst")
        .def(py::init<std::string, std::string, std::string, int, bool, std::string>(), py::arg("hostname"), py::arg("username"), py::arg("password"), py::arg("port"), py::arg("compression"), py::arg("trash_dir"))
