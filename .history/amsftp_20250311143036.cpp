        ...
        while (all_write < file_size)
        {
            ...
            if (rbuffer != "" && all_read < file_size)
            {
                do
                {
                    ...
                    rc_read = libssh2_sftp_read(srcFile, ctx.bufferd[rbuffer].bufferptr + ctx.bufferd[rbuffer].read, rchunk_size - std::cout << "rc_read: " << rc_read << std::endl;
                                                ctx.bufferd[rbuffer].read);
                    if (rc_read > 0)
                    {
                        all_read += rc_read;
                    ...
                    {
                        std::cout << get_sftp_error_msg() << std::endl;
