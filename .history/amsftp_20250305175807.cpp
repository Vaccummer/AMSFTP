...
private:
    ConRequst request;
    std::vector<std::string> private_keys;
    CircularBuffer<ED> error_info_buffer;
    AMSession *amsession;
...
public:
    ...
    AMSFTPClient(ConRequst request, std::vector<std::string> private_keys, int error_info_buffer_size = 10)
        : request(request), private_keys(private_keys)
    {
        this->amsession = new AMSession(request, private_keys);
        this->error_info_buffer = CircularBuffer<ED>(error_info_buffer_size);
    }
    ...
    SR stat(std::string path)
    {
        ...
        if (rct != 0)
        {
            ...
            error_info = {rc, "Path: " + path + " stat failed"};
            return rc;
        }
    ...
    EC exists(std::string path)
    {
        ...
        error_info = {EC::UnknownError, "Path: " + path + " stat failed"};
        return EC::UnknownError;
    }
    ...
    EC is_file(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info = {rc, "Path: " + path + " file check failed"};
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info = {EC::UnknownError, "Path: " + path + " file check failed"};
        return EC::UnknownError;
    }
    ...
    EC is_dir(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info = {rc, "Path: " + path + " dir check failed"};
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info = {EC::UnknownError, "Path: " + path + " dir check failed"};
        return EC::UnknownError;
    }
    ...
    EC is_symlink(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info = {rc, "Path: " + path + " symlink check failed"};
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info = {EC::UnknownError, "Path: " + path + " symlink check failed"};
        return EC::UnknownError;
    }
    ...
    LR listdir(std::string path)
    {
        ...
        case EC::PassCheck:
            break;
        case EC::FailedCheck:
            error_info = {EC::NotDirectory, "Path: " + path + " is not a directory"};
            return EC::NotDirectory;
        default:
            return rc;
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info = {rc, "Path: " + path + " handle open failed"};
            return rc;
        }
        ...
        while (true)
        {
            ...
            if (rct < 0)
            {
                ...
                rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
                error_info = {rc, "Path: " + path + " readdir failed"};
                return rc;
            }
            else if (rct == 0)
            {
    ...
    EC mkdir(std::string path)
    {
        ...
        case EC::PassCheck:
            return EC::Success;
        case EC::FailedCheck:
            error_info = {EC::RemoteFileExists, "Path: " + path + " exists and is not a directory"};
            return EC::RemoteFileExists;
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info = {rc, "Path: " + path + " mkdir failed"};
            return rc;
        }
        return EC::Success;
    }
