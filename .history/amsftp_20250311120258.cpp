...
private:
    ...
    EC Remote2Remote2(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        while (all_write < file_size)
        {
            ...
            if (wbuffer != "" && all_read < file_size)
            {
                libssh2_sftp_seek64(srcFile, all_read);
                rc_read = libssh2_sftp_read(srcFile, ctx.bufferd[wbuffer].bufferptr.get() + ctx.bufferd[wbuffer].written, chunk_size - ctx.bufferd[wbuffer].written);
                if (rc_read > 0)
                {
                    all_read += rc_read;
                    ctx.bufferd[wbuffer].written += rc_read;
