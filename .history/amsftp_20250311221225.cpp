...
constexpr char *AMLEVEL = "AMLEVEL";
constexpr char *AMERRORNAME = "AMERRORNAME";
constexpr char *AMTARGET = "AMTARGET";
constexpr char *AMACTION = "AMACTION";
constexpr char *AMMESSAGE = "AMMESSAGE";

constexpr char *AMCRITICAL = "AMCRITICAL";
...
public:
    ...
    SR stat(std::string path)
    {
        ...
        if (rct != 0)
        {
            ...
            {
                pytrace(WARNING, "StatFailed", fmt::format("{}@{}", request.nickname, path), "Stat", get_session_error_msg());
            }
            return rc;
        }
    ...
    LR listdir(std::string path)
    {
        ...
        if (!sftp_handle)
        {
            pytrace(WARNING, "HandleOpenFailed", fmt::format("{}@{}", request.nickname, path), "ListDir", get_session_error_msg());
            goto clean;
        }
    ...
    EC mkdir(std::string path)
    {
        ...
        if (rcr != 0)
        {
            ...
            {
                pytrace(WARNING, "MkdirFailed", fmt::format("{}@{}", request.nickname, path), "Mkdir", get_session_error_msg());
            }
            return rc;
        }
        return EC::Success;
    ...
    EC rmfile(std::string path)
    {
        ...
        if (rcr != 0)
        {
            ...
            if (rc != EC::PermissionDenied && rc != EC::PathNotExist)
            {
                pytrace(WARNING, "RmfileFailed", fmt::format("{}@{}", request.nickname, path), "Rmfile", get_session_error_msg());
            }
            return rc;
        }
        return EC::Success;
    ...
    EC rmdir(std::string path)
    {
        ...
        if (rcr < 0)
        {
            ...
            if (rc != EC::PermissionDenied && rc != EC::PathNotExist)
            {
                pytrace(WARNING, "RmdirFailed", fmt::format("{}@{}", request.nickname, path), "Rmdir", get_session_error_msg());
            }
            return rc;
        }
        return EC::Success;
    ...
    EC move(std::string src, std::string dst, bool need_mkdir = false, bool force_write = false)
    {
        ...
        if (rcr != 0)
        {
            ...
            {
                pytrace(WARNING, "MoveFailed", fmt::format("{}@{}->{}", request.nickname, src, dst_path), "Move", get_session_error_msg());
            }
            return rc;
        }
    ...
    EC rename(std::string src, std::string dst, bool need_mkdir = false, bool force_write = false)
    {
        ...
        if (rcr != 0)
        {
            ...
            if (rc != EC::PermissionDenied && rc != EC::PathNotExist && rc != EC::TargetExists)
            {
                pytrace(WARNING, "RenameFailed", fmt::format("{}@{}->{}", request.nickname, src, dst), "Rename", get_session_error_msg());
            }
            return rc;
        }
        return EC::Success;
...
private:
    ...
    EC Local2Remote(std::string src, std::string dst)
    {
        ...
        if (!sftpFile)
        {
            ...
            if (rct == LIBSSH2_FX_NO_SUCH_FILE)
            {
                ...
                if (!sftpFile)
                {
                    pytrace(ERROR, "RemoteFileOpenError", fmt::format("{}@{}", request.nickname, dst), "Remote2Local", get_session_error_msg());
        ...
        if (!l_file_m.file_ptr)
        {
            ...
            pytrace(ERROR, "LocalFileMapError", fmt::format("Local@{}", src), "Local2Remote", error_msg);
    ...
    clean:
        ...
        case EC::Success:
            break;
        case EC::Terminate:
            pytrace(INFO, "", fmt::format("Local@{}->{}@{}", src, request.nickname, dst), "Local2Remote", "Transfer cancelled");
            break;
        case EC::UnexpectedEOF:
            pytrace(ERROR, "UnexpectedEOF", fmt::format("Local@{}->{}@{}", src, request.nickname, dst), "Local2Remote", "File was unexpectedly truncated");
            break;
        default:
            pytrace(ERROR, "TransferError", fmt::format("Local@{}->{}@{}", src, request.nickname, dst), "Local2Remote", get_session_error_msg());
            break;
        }
        return rc_r;
    }
    ...
    EC Remote2Local(std::string src, std::string dst)
    {
        ...
        if (!sftpFile)
        {
            pytrace(ERROR, "RemoteFileOpenError", fmt::format("{}@{}", request.nickname, src), "Remote2Local", get_session_error_msg());
            return EC::RemoteFileOpenError;
        }
        LIBSSH2_SFTP_ATTRIBUTES attrs;
        ...
        {
            libssh2_sftp_close_handle(sftpFile);
            pytrace(ERROR, "RemoteFileStatError", fmt::format("{}@{}", request.nickname, src), "Remote2Local", get_session_error_msg());
            return EC::PathNotExist;
        }
        ...
        {
            libssh2_sftp_close_handle(sftpFile);
            pytrace(ERROR, "LocalFileMapError", fmt::format("Local@{}", dst), "Remote2Local", error_msg);
    ...
    clean:
        ...
        case EC::Success:
            break;
        case EC::Terminate:
            pytrace(INFO, "", fmt::format("{}@{}->Local@{}", request.nickname, src, dst), "Remote2Local", "Transfer cancelled");
            break;
        case EC::UnexpectedEOF:
            pytrace(ERROR, "UnexpectedEOF", fmt::format("{}@{}->Local@{}", request.nickname, src, dst), "Remote2Local", "File was unexpectedly truncated");
            break;
        default:
            pytrace(ERROR, "TransferError", fmt::format("{}@{}->Local@{}", request.nickname, src, dst), "Remote2Local", get_session_error_msg());
            break;
        }
        return rc_r;
    }
    ...
    EC Remote2Remote_ori(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        if (!srcFile)
        {
            pytrace(ERROR, "RemoteSrcOpenError", fmt::format("{}@{}", request.nickname, src), "Remote2Remote", get_session_error_msg());
            return get_sftp_last_error();
        }
        if (!dstFile)
        {
            ...
            if (rct == LIBSSH2_FX_NO_SUCH_FILE)
            {
                ...
                if (!dstFile)
                {
                    pytrace(ERROR, "RemoteDstOpenError", fmt::format("{}@{}", request.nickname, dst), "Remote2Remote", get_session_error_msg());
                    return EC::RemoteFileOpenError;
                }
            }
            else
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        if (!srcFile)
        {
            std::string tar = get_session_error_msg();
            std::cout << "tar: " << tar << std::endl;
            // std::string msg = sformat("{}@{}", request.nickname, src);
            pytrace(ERROR, "RemoteSrcOpenError", "haha", "Remote2Remote", "");
            return EC::RemoteFileOpenError;
        }
        ...
        if (!dstFile)
        {
            ...
            if (rct == LIBSSH2_FX_NO_SUCH_FILE)
            {
                std::cout << "dstFile: " << "3" << std::endl;
                another_worker->mkdirs(dirname(dst));
                dstFile = libssh2_sftp_open(another_worker->amsession->sftp, dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC, 0744);
                std::cout << "dstFile: " << "4" << std::endl;
                if (!dstFile)
                {
                    std::cout << "dstFile: " << "5" << std::endl;
                    pytrace(ERROR, "RemoteDstOpenError", fmt::format("{}@{}", request.nickname, dst), "Remote2Remote", get_session_error_msg());
                    return EC::RemoteFileOpenError;
                }
            }
