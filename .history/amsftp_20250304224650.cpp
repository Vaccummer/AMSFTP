...
public:
    ...
    EC saferm(std::string path)
    {
        ...
        {
            target_path = join_path(request.trash_dir, std::to_string(i) + "_" + base);
            i++;
        }
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<PathInfo>(m, "PathInfo")
        .def(py::init<std::string, std::string, std::string, std::string, uint64_t, uint64_t, PathType>(), py::arg("name"), py::arg("path"), py::arg("dir"), py::arg("owner"), py::arg("group"), py::arg("size"), py::arg("path_type"))
