...
private:
    ...
    {
        std::lock_guard<std::mutex> lock(amsession->mutex_c);
...
private:
    ...
    {
        std::lock_guard<std::mutex> lock(dst_client->amsession->mutex_c);
    ...
    {
        std::lock_guard<std::mutex> lock(src_client->amsession->mutex_c);
        TransferErrorCode rc_r = EC::Success;
        LIBSSH2_SFTP_HANDLE *sftpFile;
    ...
    {
        std::lock_guard<std::mutex> lock(dst_client->amsession->mutex_c);
        std::lock_guard<std::mutex> lock_src(src_client->amsession->mutex_c);
...
public:
    ...
    EC check_client()
    {
        EC rc = EC::ConnectionCheckFailed;
        if (src_client)
        {
            rc = src_client->check();
            if (rc != EC::Success)
