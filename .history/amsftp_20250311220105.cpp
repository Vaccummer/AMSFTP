...
std::string sformat(const std::string &format, const std::string &args)
{
    std::string outstr = fmt::format(format, args);
    return outstr;
}
...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        if (!srcFile)
        {
            ...
            pytrace(ERROR, "RemoteSrcOpenError", fmt::format("{}@{}", request.nickname, src), "Remote2Remote", tar);
            return EC::RemoteFileOpenError;
        }
...
public:
    ...
    void pause()
    {
        is_pause.store(true, std::memory_order_release);
    }
