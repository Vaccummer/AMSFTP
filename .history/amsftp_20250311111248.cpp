...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        if (!srcFile)
        {
            pytrace(ERROR, "RemoteFileOpenError", fmt::format("{}@{}", request.nickname, src), "Remote2Remote", get_sftp_error_msg());
            return get_sftp_last_error();
        }
        if (!dstFile)
        {
            pytrace(ERROR, "RemoteFileOpenError", fmt::format("{}@{}", request.nickname, dst), "Remote2Remote", get_sftp_error_msg());
