...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        struct timeval timeout_transfer;
        timeout_transfer.tv_sec = 10;
        timeout_transfer.tv_usec = 0;
        FD_SET(amsession->sock, &read_fds);
        FD_SET(another_worker->amsession->sock, &write_fds);
        FD_ZERO(&read_fds);
        FD_ZERO(&write_fds);
        std::cout << another_worker->amsession->sock << std::endl;
        ...
        while (all_write < file_size)
        {
            ...
            if (eagain_count > 1e5)
            {
                FD_ZERO(&read_fds);
                FD_ZERO(&write_fds);
                int ret = select(0, &read_fds, &write_fds, NULL, &timeout_transfer);
                if (ret < 0)
                {
