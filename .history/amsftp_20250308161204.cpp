...
public:
    ...
    void record_error(EC error, std::string src, std::string dst, std::string function_name, std::string msg)
    {
        std::lock_guard<std::recursive_mutex> lock(r_mtx);
        error_info_buffer.push(ErrorInfo(error, src, dst, function_name, msg));
    }
    ...
    SIZER get_size(std::string path, bool exclude_dir = false, bool exclude_symlink = false)
    {
        ...
        if (error != EC::Success)
        {
            ...
            error_info_buffer.push(ErrorInfo(error, path, "", "AMSFTPClient::get_size", "get size failed"));
            return error;
        }
        return size;
    }
