...
struct SingleBuffer
{
    ...
    int status = 0; // 0:ready for write, 1: ready for read
    uint64_t written = 0;
    uint64_t read = 0;
    SingleBuffer(size_t buffer_size) : bufferptr(std::make_shared<char[]>(buffer_size)) {}
};
struct TransferContext
{
    ...
    {
        if (bufferd["a"].status == 0)
        {
            return "a";
        }
        else if (bufferd["b"].status == 0)
        {
            return "b";
        }
        else
    ...
    {
        if (bufferd["a"].status == 1)
        {
            return "a";
        }
        else if (bufferd["b"].status == 1)
        {
            return "b";
        }
        else
...
private:
    ...
    EC Remote2Remote2(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        TransferErrorCode rc_final = EC::Success;
        long rct;
        LIBSSH2_SFTP_HANDLE *srcFile;
        ...
        uint64_t count = 0;
        uint64_t rchunk_size = std::min<uint64_t>(buffer_size, file_size - offset);
        uint64_t wchunk_size = std::min<uint64_t>(buffer_size, file_size - offset);
        uint64_t all_read = 0;
        ...
        while (all_write < file_size)
        {
            ...
            if (rbuffer != "" && all_read < file_size)
            {
                libssh2_sftp_seek64(srcFile, all_read);
                rc_read = libssh2_sftp_read(srcFile, ctx.bufferd[rbuffer].bufferptr.get() + ctx.bufferd[rbuffer].read, rchunk_size - ctx.bufferd[rbuffer].read);
                if (rc_read > 0)
                {
                    all_read += rc_read;
                    ...
                    rchunk_size = std::min<uint64_t>(buffer_size, file_size - offset);
                }
                else if (rc_read == 0)
                {
                    if (ctx.bufferd[rbuffer].read == chunk_size)
                    {
                        ctx.bufferd[rbuffer].status = 1;
                        ctx.bufferd[rbuffer].read = 0;
                    }
            ...
            if (wbuffer != "" && all_write < file_size)
            {
                ...
                rc_write = libssh2_sftp_write(dstFile, ctx.bufferd[wbuffer].bufferptr.get() + ctx.bufferd[wbuffer].written, chunk_size - ctx.bufferd[wbuffer].written);
                if (rc_write > 0)
                {
                    all_write += rc_write;
                    ctx.bufferd[wbuffer].written += rc_write;
                    if (ctx.bufferd[wbuffer].written == chunk_size)
                    {
                        ctx.bufferd[wbuffer].status = 1;
