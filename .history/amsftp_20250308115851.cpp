...
void _walk(std::string path, WRV &result)
{
    ...
    if (fs::exists(p))
    {
        ...
        uint64_t atime = std::chrono::duration_cast<std::chrono::seconds>(fs::last_write_time(p).time_since_epoch()).count();
        uint64_t mtime = std::chrono::duration_cast<std::chrono::seconds>(fs::last_write_time(p).time_since_epoch()).count();
        ...
        {
            result.push_back(PathInfo(p.filename().string(), p.string(), p.parent_path().string(), 0, 0, 0, PathType::SYMLINK));
        }
        else
        {
            result.push_back(PathInfo(p.filename().string(), p.string(), p.parent_path().string(), fs::file_size(p), fs::last_write_time(p), fs::last_write_time(p), PathType::FILE));
        }
    }
}
...
{
private:
    // TODO:Inherit from AMSFTPClient, implement the self-defined upload method
    TransferCallback callback;
    ConRequst src_request;
    ConRequst dst_request;
    AMSFTPClient *src_client = nullptr;
    AMSFTPClient *dst_client = nullptr;
    TASKS tasks;
...
public:
        ...
        : ID(ID), src_request(src_request), dst_request(dst_request), callback(callback), tasks(tasks)
    {
        ...
        {
            this->src_client = new AMSFTPClient(src_request, private_keys);
        }
        if (!dst_request.hostname.empty())
        {
            this->dst_client = new AMSFTPClient(dst_request, private_keys);
        }
    }

    EC init()
    {
        EC rc = EC::NoSFTPConnection;
        if (src_client)
        {
            rc = src_client->init();
            if (rc != EC::Success)
