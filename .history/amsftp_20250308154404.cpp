...
public:
    ...
    AMSFTPWorker(std::vector<std::string> private_keys, ConRequst request)
        : nickname(nickname), request(request), callback(callback)
    {
        if (!request.hostname.empty())
        {
            this->client = new AMSFTPClient(request, private_keys);
        }
        if (!dst_request.hostname.empty())
        {
            this->dst_client = new AMSFTPClient(dst_request, private_keys);
