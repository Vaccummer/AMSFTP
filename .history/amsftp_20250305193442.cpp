...
public:
    ...
    EC copy(std::string src, std::string dst, bool need_mkdir = false)
    {
        ...
        if (rcr != 0)
        {
            error_info_buffer.push(ErrorInfo(rc, src, dst, "AMSFTPClient::copy", "Channel error"));
            return rc;
        }
        int exit_code = libssh2_channel_get_exit_status(amsession->channel);
        if (exit_code != 0)
        {
            error_info_buffer.push(ErrorInfo(EC::CopyError, src, dst, "AMSFTPClient::copy", "Cmd exec failed"));
            return EC::CopyError;
        }
        return EC::Success;
    };
