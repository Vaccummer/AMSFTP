...
public:
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(t_mtx);
...
private:
    ...
    std::recursive_mutex transfer_mtx;
    ...
    EC Local2Remote(std::string src, std::string dst)
    {
        ...
        if (!sftpFile)
        {
            ...
            {
                libssh2_sftp_mkdir(amsession->sftp, dirname(dst).c_str(), 0744);
                sftpFile = libssh2_sftp_open(amsession->sftp, dst.c_str(), LIBSSH2_FXF_TRUNC | LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT, 0744);
                if (!sftpFile)
                {
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPClient> another_client)
    {
        std::lock_guard<std::recursive_mutex> lock_src(amsession->r_mtx);
        std::lock_guard<std::recursive_mutex> lock_dst(another_client->amsession->r_mtx);
        ...
        dstFile = libssh2_sftp_open(another_client->amsession->sftp, dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC, 0744);
        ...
        if (!dstFile)
        {
            rct = libssh2_sftp_last_error(another_client->amsession->sftp);
            if (rct == LIBSSH2_FX_NO_SUCH_FILE)
            {
                libssh2_sftp_mkdir(another_client->amsession->sftp, dirname(dst).c_str(), 0744);
                dstFile = libssh2_sftp_open(another_client->amsession->sftp, dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC, 0744);
                if (!dstFile)
                {
                    return EC::RemoteFileOpenError;
        ...
        while (offset < file_size)
        {
            ...
            while (local_write < chunk_size)
            {
                ...
                {
                    rc_final = cast_libssh2_error(libssh2_sftp_last_error(another_client->amsession->sftp));
                    goto clean;
                }
            }
...
public:
    ...
    TR upload(std::string src, std::string dst, bool force_write = false, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        std::lock_guard<std::recursive_mutex> lock(transfer_mtx);
        EC rc = check();
        if (rc != EC::Success)
        {
            return rc;
        ...
        if (!fs::is_directory(fs::path(src)))
        {
            if (basename(src) == basename(dst))
            {
                if (exists(dirname(dst)) != EC::PassCheck)
                {
                    rc = mkdirs(dirname(dst));
                    if (rc != EC::Success)
                    {
                        return rc;
                    }
            ...
            {
                mkdirs(dst);
                if (is_dir(dst) != EC::PassCheck)
                {
                    return EC::NotDirectory;
                }
                dst = join_path(dst, basename(src));
            }
            if (exists(dst) == EC::PassCheck)
            {
                if (!force_write)
                {
                    return EC::DirAlreadyExists;
                }
                else
            }
            ...
            {
                py::gil_scoped_acquire acquire;
                callback.filename_cb(dst);
            }

            rc = Local2Remote(src, dst);
            result[dst] = rc;

            if (rc != EC::Success && callback.need_error_cb)
            {
                py::gil_scoped_acquire acquire;
                callback.error_cb(dst, rc);
            }
            return result;
        }
        ...
        std::vector<PathInfo> src_all = local_walk(src);
        fs::path root_path = fs::path(src);
        fs::path src_path_i;
        std::string dst_path_i;
        fs::path relative_path;
        uint64_t t_size = 0;
        TASKS dir_tasks;
        TASKS file_tasks;

        for (auto &path_info : src_all)
        {
            current_filename = path_info.path;
            src_path_i = fs::path(path_info.path);
            relative_path = fs::relative(src_path_i, root_path);
            dst_path_i = join_path(dst, relative_path);
            t_size += path_info.size;

            if (path_info.path_type == PathType::DIR)
            {
                dir_tasks.push_back(TransferTask(current_filename, dst_path_i, path_info.size));
            }
            else
            {
                file_tasks.push_back(TransferTask(current_filename, dst_path_i, path_info.size));
            }

            total_size = t_size;

            for (auto &task : dir_tasks)
            {
                current_filename = task.src;
                if (callback.need_filename_cb)
                {
                    py::gil_scoped_acquire acquire;
                    callback.filename_cb(task.dst);
                }
                rc = mkdirs(task.dst);
            ...
            for (auto &task : file_tasks)
            {
                ...
                {
                    py::gil_scoped_acquire acquire;
                    callback.filename_cb(task.src);
                }
                rc = Local2Remote(task.src, task.dst);
                result[task.dst] = rc;
                if (rc != EC::Success && callback.need_error_cb)
                {
                    py::gil_scoped_acquire acquire;
            ...
            TR download(std::string src, std::string dst, bool force_write = false, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
            {
                std::lock_guard<std::recursive_mutex> lock(transfer_mtx);
                EC rc = check();
                if (rc != EC::Success)
                ...
                for (auto &path_info : src_all)
                {
                    ...
                    if (path_info.path_type == PathType::DIR)
                    {
                        rc = mkdirs(dst_path_i);
                        if (rc == EC::Success)
                        {
                            current_size += path_info.size;
                        }
                        result[dst_path_i] = rc;
                    }
                    else
                    {

                        rc = Local2Remote(current_filename, dst_path_i);
                        result[current_filename] = rc;
                    }
            ...
            TR toAnotherHost(std::string src, std::string dst, std::shared_ptr<AMSFTPClient> another_client)
            {
                std::lock_guard<std::recursive_mutex> lock(transfer_mtx);
                TRM result = {};
                EC rc = check();
                ...
                for (auto &task : tasks)
                {
                    ...
                    EC our_i = Remote2Remote(task.src, task.dst, another_client);
