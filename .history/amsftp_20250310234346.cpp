...
public:
    ...
    EC ensure_trash_dir()
    {
        ...
        {
            std::string target = fmt::format("{}@{}", request.nickname, request.port);
            pytrace(CRITICAL, "TrashDirCreateFailed", target, "EnsureTrashDir", "Failed");
            return rc;
        }
        return EC::Success;
    }
    ...
    SR stat(std::string path)
    {
        ...
        if (rct != 0)
        {
            ...
            pytrace(CRITICAL, "StatFailed", fmt::format("{}@{}", request.nickname, request.port), "Stat", get_sftp_error_msg());
            return rc;
        }
    ...
    LR listdir(std::string path)
    {
        ...
        {
            std::string target = fmt::format("{}@{}", request.nickname, request.port);
            pytrace(WARNING, "HandleOpenFailed", target, "OpenDir", get_sftp_error_msg());
            goto clean;
        }
    ...
    EC mkdir(std::string path)
    {
        ...
        if (rcr != 0)
        {
            ...
            pytrace(CRITICAL, "MkdirFailed", fmt::format("{}@{}", request.nickname, path), "Mkdir", get_sftp_error_msg());
            return rc;
        }
        return EC::Success;
    }
