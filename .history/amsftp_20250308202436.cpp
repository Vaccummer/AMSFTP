...
public:
    ...
    TR upload(std::string src, std::string dst, bool force_write = false, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        ...
        for (auto &path_info : src_all)
        {
            ...
            else
            {
                ...
                EC our_i = Local2Remote(current_filename, dst_path_i);
                result[current_filename] = our_i;
            }
    ...
    TR download(std::string src, std::string dst, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
        std::lock_guard<std::recursive_mutex> lock(transfer_mtx);
        TRM result = {};
        EC rc = check();
