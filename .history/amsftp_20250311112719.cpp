...
{
    std::shared_ptr<char[]> buf_a;
    std::shared_ptr<char[]> buf_b;
    int a_status = 0; // 0:ready for write, 1: ready for read, 2: is_writing, 3: is_reading
    int b_status = 0; // 0:ready for write, 1: ready for read, 2: is_writing, 3: is_reading
    uint64_t a_written = 0;
    uint64_t b_written = 0;
    uint64_t a_read = 0;
    uint64_t b_read = 0;
    TransferContext(size_t buffer_size) : buf_a(std::make_shared<char[]>(buffer_size)),
                                          buf_b(std::make_shared<char[]>(buffer_size)) {}
};
...
private:
    ...
    EC Remote2Remote2(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        while (offset < file_size)
        {
            ...
            chunk_size = std::min<uint64_t>(buffer_size_ori, file_size - offset);
            if (ctx.a_writing_ready || ctx.b_writing_ready)
            {
                if (ctx.a_writing_ready)
                {
                    local_write = 0;
                }
                else
