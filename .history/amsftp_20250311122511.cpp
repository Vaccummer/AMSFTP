...
struct SingleBuffer
{
    ...
    uint64_t written = 0;
    uint64_t read = 0;
    SingleBuffer(size_t buffer_size) : bufferptr_origin(std::make_shared<char[]>(new char[buffer_size])), bufferptr(&bufferptr_origin.get()) {}
};
...
private:
    ...
    EC Remote2Remote2(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        while (all_write < file_size)
        {
            ...
            if (rbuffer != "" && all_read < file_size && FD_ISSET(amsession->sock, &read_fds))
            {
            ...
            if (wbuffer != "" && all_write < file_size && FD_ISSET(another_worker->amsession->sock, &write_fds))
            {
                libssh2_sftp_seek64(dstFile, all_write);
                rc_write = libssh2_sftp_write(dstFile, ctx.bufferd[wbuffer].bufferptr.get() + ctx.bufferd[wbuffer].written, wchunk_size - ctx.bufferd[wbuffer].written);
                if (rc_write > 0)
                {
                    all_write += rc_write;
