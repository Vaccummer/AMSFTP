...
struct SingleBuffer
{
    ...
    int status = 0; // 0:ready for write, 1: ready for read, 2: is_writing, 3: is_reading
    uint64_t written = 0;
    uint64_t read = 0;
...
{
    std::unordered_map<std::string, std::shared_ptr<char[]>> bufferd;
    std::unordered_map<std::string, int> statusd;
