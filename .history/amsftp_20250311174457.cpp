...
struct TransferContext
{
    ...
    int get_write_buffer()
    {
        ...
        {
            if (bufferd[0].read_order > bufferd[1].read_order)
    ...
    void finish_write(int buffer_name, uint64_t priority)
    {
        bufferd[buffer_name].status = BufferStatus::write_done;
        bufferd[buffer_name].written = 0;
        bufferd[buffer_name].read = 0;
        bufferd[buffer_name].write_priority = priority;
    }

    void finish_read(int buffer_name, uint64_t priority)
    {
        bufferd[buffer_name].status = BufferStatus::read_done;
        bufferd[buffer_name].written = 0;
        bufferd[buffer_name].read_order = priority;
    }
};
