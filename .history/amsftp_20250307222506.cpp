...
struct TransferContext
{
    ...
    TransferContext(uint64_t buffer_size, uint64_t fs) : file_size(fs)
    {
...
private:
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        const int src_sock = src_session->sock;
        const int dst_sock = dst_session->sock;
        const int maxfd = std::max<int>(src_sock, dst_sock) + 1;
