...
public:
    ...
    EC check()
    {
        ...
        {
            rc = EC::SessionBroken;
            error_info_buffer.push({rc, "Session is not initialized"});
            return rc;
        }
        else
        {
            ...
            if (rc != EC::Success)
            {
                error_info_buffer.push({rc, "Session is broken"});
            }
        }
        return rc;
    }
    ...
    EC reconnect()
    {
        ...
        if (rc != EC::Success)
        {
            error_info_buffer.push({rc, "Session init failed"});
        }
        return rc;
    }
    ...
    EC ensure()
    {
        ...
        if (rc != EC::Success)
        {
            ...
            if (rc != EC::Success)
            {
                error_info_buffer.push({rc, "Session reconnect failed"});
                return rc;
            }
        }
        return rc;
    ...
    EC ensure_trash_dir()
    {
        ...
        if (std::holds_alternative<EC>(info))
        {
            ...
            {
                error_info_buffer.push({rc, "Trash dir initializes failed"});
                return rc;
            }
        }
        ...
        if (std::holds_alternative<PathInfo>(info))
        {
            ...
            {
                error_info_buffer.push({EC::TargetNotADirectory, "Trash dir: " + request.trash_dir + " is not a directory"});
        ...
        if (rc != EC::Success)
        {
            error_info_buffer.push({rc, "Trash dir: " + request.trash_dir + " mkdirs failed"});
            return rc;
        }
        return EC::Success;
    }
    ...
    EC init()
    {
        ...
        if (rc != EC::Success)
        {
            error_info_buffer.push({rc, "Session check failed"});
            return rc;
        }
        return rc;
    }
    ...
    SR stat(std::string path)
    {
        ...
        if (rct != 0)
        {
            ...
            error_info_buffer.push({rc, "Path: " + path + " stat failed"});
            return rc;
        }
    ...
    EC exists(std::string path)
    {
        ...
        error_info_buffer.push({EC::UnknownError, "Path: " + path + " stat failed"});
        return EC::UnknownError;
    }
    ...
    EC is_file(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info_buffer.push({rc, "Path: " + path + " file check failed"});
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info_buffer.push({EC::UnknownError, "Path: " + path + " file check failed"});
        return EC::UnknownError;
    }
    ...
    EC is_dir(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info_buffer.push({rc, "Path: " + path + " dir check failed"});
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info_buffer.push({EC::UnknownError, "Path: " + path + " dir check failed"});
        return EC::UnknownError;
    }
    ...
    EC is_symlink(std::string path)
    {
        ...
        {
            rc = std::get<EC>(info);
            error_info_buffer.push({rc, "Path: " + path + " symlink check failed"});
            return rc;
        }
        if (std::holds_alternative<PathInfo>(info))
        {
        ...
        error_info_buffer.push({EC::UnknownError, "Path: " + path + " symlink check failed"});
        return EC::UnknownError;
    }
    ...
    LR listdir(std::string path)
    {
        ...
        case EC::PassCheck:
            break;
        case EC::FailedCheck:
            error_info_buffer.push({EC::NotDirectory, "Path: " + path + " is not a directory"});
            return EC::NotDirectory;
        default:
            return rc;
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + path + " handle open failed"});
            return rc;
        }
        ...
        while (true)
        {
            ...
            if (rct < 0)
            {
                ...
                rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
                error_info_buffer.push({rc, "Path: " + path + " readdir failed"});
                return rc;
            }
            else if (rct == 0)
            {
    ...
    EC mkdir(std::string path)
    {
        ...
        case EC::PassCheck:
            return EC::Success;
        case EC::FailedCheck:
            error_info_buffer.push({EC::RemoteFileExists, "Path: " + path + " exists and is not a directory"});
            return EC::RemoteFileExists;
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + path + " mkdir failed"});
            return rc;
        }
        return EC::Success;
    }
    ...
    EC rmfile(std::string path)
    {
        ...
        if (std::holds_alternative<PathInfo>(info))
        {
            ...
            {
                error_info_buffer.push({EC::TargetNotAFile, "Path: " + path + " rmfile failed"});
                return EC::TargetNotAFile;
            }
        }
        else
        {
            error_info_buffer.push({EC::UnknownError, "Path: " + path + " rmfile failed"});
            return EC::UnknownError;
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + path + " rmfile failed"});
            return rc;
        }
        return EC::Success;
    }
    ...
    EC rmdir(std::string path)
    {
        ...
        case EC::FailedCheck:
            return EC::TargetNotADirectory;
        default:
            error_info_buffer.push({rc, "Path: " + path + " rmdir failed"});
            return rc;
        }
    ...
    EC rm(std::string path)
    {
        ...
        case EC::FailedCheck:
            ...
            break;
        default:
            error_info_buffer.push({rc, "Path: " + path + " rm failed"});
            return rc;
        }
    ...
    EC saferm(std::string path)
    {
        ...
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push({rc, "Path: " + path + " saferm failed because path not exists"});
            return rc;
        }
        ...
        if (rc != EC::PassCheck)
        {
            ...
            if (rc != EC::Success)
            {
                error_info_buffer.push({rc, "Path: " + path + " saferm failed because trash_dir: " + request.trash_dir + " mkdir failed"});
                return rc;
            }
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + path + " saferm failed because rename failed"});
            return rc;
        }
        return EC::Success;
    }
    ...
    EC move(std::string src, std::string dst, bool need_mkdir = false, bool force_write = false)
    {
        ...
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push({rc, "Path: " + src + " move failed because src not exists"});
            return rc;
        }
        ...
        if (rc != EC::PassCheck)
        {
            if (need_mkdir)
            {
                ...
                if (rc != EC::Success)
                {
                    error_info_buffer.push({rc, "Path: " + src + " move failed because dst_dir: " + dst_dir + " mkdir failed"});
                    return rc;
                }
            }
            else
            {
                error_info_buffer.push({EC::ParentDirectoryNotExist, "Path: " + src + " move failed because dst_dir: " + dst_dir + " not exists"});
        ...
        if (exists(dst_path) == EC::PassCheck)
        {
            if (!force_write)
            {
                error_info_buffer.push({EC::TargetExists, "Path: " + src + " move failed because dst_path: " + dst_path + " exists"});
                return EC::TargetExists;
            }
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + src + " move failed because rename failed"});
            return rc;
        }
    ...
    EC copy(std::string src, std::string dst, bool need_mkdir = false)
    {
        ...
        case EC::PassCheck:
            break;
        case EC::FailedCheck:
            error_info_buffer.push({rc, "Path: " + src + " copy failed because src not exists"});
            return EC::PathNotExist;
        default:
            error_info_buffer.push({rc, "Path: " + src + " copy failed because src check failed"});
            return rc;
        }
        ...
        if (rc != EC::PassCheck)
        {
            if (need_mkdir)
            {
                ...
                if (rc != EC::Success)
                {
                    error_info_buffer.push({rc, "Path: " + src + " copy failed because dst_dir: " + dst + " mkdir failed"});
                    return rc;
                }
            }
            else
            {
                error_info_buffer.push({EC::ParentDirectoryNotExist, "Path: " + src + " copy failed because dst_dir: " + dst + " not exists"});
                return EC::ParentDirectoryNotExist;
            }
        }
        ...
        if (rcr != 0)
        {
            error_info_buffer.push({rc, "Path: " + src + "->" + dst + " copy failed because channel error"});
            return rc;
        }
        int exit_code = libssh2_channel_get_exit_status(amsession->channel);
        if (exit_code != 0)
        {
            error_info_buffer.push({EC::CopyError, "Path: " + src + "->" + dst + " copy failed because cmd exec failed"});
            return EC::CopyError;
        }
        return EC::Success;
    };
    ...
    EC rename(std::string src, std::string dst, bool need_mkdir = false, bool force_write = false)
    {
        ...
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push({rc, "Path: " + src + " rename failed because src not exists"});
            return EC::PathNotExist;
        }
        rc = exists(dst);
        switch (rc)
        ...
        case EC::PassCheck:
            if (!force_write)
            {
                error_info_buffer.push({EC::TargetExists, "Path: " + src + " rename failed because dst: " + dst + " exists"});
                return EC::TargetExists;
            }
            break;
        case EC::FailedCheck:
            break;
        default:
            error_info_buffer.push({rc, "Path: " + src + " rename failed because dst check failed"});
            return rc;
        }
        dst_dir = dirname(dst);
        ...
        if (rc != EC::PassCheck)
        {
            if (need_mkdir)
            {
                ...
                if (rc != EC::Success)
                {
                    error_info_buffer.push({rc, "Path: " + src + " rename failed because dst_dir: " + dst_dir + " mkdir failed"});
                    return rc;
                }
            }
            else
            {
                error_info_buffer.push({EC::ParentDirectoryNotExist, "Path: " + src + " rename failed because dst_dir: " + dst_dir + " not exists"});
                return EC::ParentDirectoryNotExist;
            }
        }
        ...
        {
            rc = cast_libssh2_error(libssh2_sftp_last_error(amsession->sftp));
            error_info_buffer.push({rc, "Path: " + src + "->" + dst + " rename failed"});
            return rc;
        }
        return EC::Success;
    }
    ...
    WR walk(std::string path)
    {
        ...
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push({rc, "Path: " + path + " walk failed because path not exists"});
            return rc;
        }
        rc = is_dir(path);
        if (rc != EC::PassCheck)
        {
            error_info_buffer.push({rc, "Path: " + path + " walk failed because path is not a directory"});
            return rc;
        }
        _walk(path, result);
        return result;
    ...
    ED get_last_error_info()
    {
        return error_info_buffer.back();
    }

    std::vector<ED> get_all_error_info()
    {
...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<AMSFTPClient>(m, "AMSFTPClient")
        ...
        .def("get_error_info", &AMSFTPClient::get_error_info)
        .def("get_error_info_buffer", &AMSFTPClient::get_error_info_buffer);
}
