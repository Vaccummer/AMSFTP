...
private:
    ...
    EC Local2Remote(const TransferTask &task)
    {
        ...
        if (set.force_write)
        {
            sftpFile = libssh2_sftp_open(dst_session->sftp, task.dst.c_str(), LIBSSH2_FXF_TRUNC | LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT, 0740);
        }
        else
        {
            sftpFile = libssh2_sftp_open(dst_session->sftp, task.dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT, 0740);
        }
        if (!sftpFile)
        {
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        if (set.force_write)
        {
            dstFile = libssh2_sftp_open(dst_session->sftp, task.dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT | LIBSSH2_FXF_TRUNC, 0740);
        }
        else
        {
            dstFile = libssh2_sftp_open(dst_session->sftp, task.dst.c_str(), LIBSSH2_FXF_WRITE | LIBSSH2_FXF_CREAT, 0740);
...
public:
    ...
    AMSFTPWorker(uint64_t ID, std::vector<std::string> private_keys, TransferSet set, TransferCallback callback, TASKS tasks, ConRequst src_request = ConRequst(), ConRequst dst_request = ConRequst())
        : ID(ID), src_request(src_request), dst_request(dst_request), set(set), callback(callback), tasks(tasks)
    {
