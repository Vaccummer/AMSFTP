...
struct BufferSet
{
    ...
    BufferSet()
        : buffer_sizes({BufferSizePair{16 * AMGB, 8 * AMMB},
...
private:
    ...
    EC Remote2Local(std::string src, std::string dst)
    {
        ...
        SR stat_result = stat(src);
        if (!std::holds_alternative<PathInfo>(stat_result))
        {
            libssh2_sftp_close_handle(sftpFile);
            if (std::holds_alternative<TransferErrorCode>(stat_result))
                return std::get<TransferErrorCode>(stat_result);
            else
                return EC::UnknownError;
        }
        uint64_t file_size = std::get<PathInfo>(stat_result).size;
