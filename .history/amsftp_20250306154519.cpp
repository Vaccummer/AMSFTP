    ...
    EC Local2Remote(const TransferTask &task)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (rc > 0)
            {
                ...
                if (speed_interval_cout % adjust_interval == 0)
                {
                    if (time_end - time_start < 0.001)
                    {
                        speed = 32 * AMGB;
                    }
                    else
                    {
                        speed = static_cast<double>(chunk_size) / (time_end - time_start > 0 ? time_end - time_start : 0.000001);
                        std::cout << "speed: " << speed / AMMB << "MB/s" << std::endl;
                    }
    ...
    EC Remote2Local(const TransferTask &task)
    {
        ...
        while (offset < file_size)
        {
            ...
            if (rc > 0)
            {
                ...
                if (speed_interval_cout % adjust_interval == 0)
                {
                    interval_time = time_end - time_start > 0.001 ? time_end - time_start : 0.001;
                    if (interval_time < 0.01)
                    {
                        speed = 32 * AMGB;
                    }
                    else
                    {
                        speed = static_cast<double>(chunk_size) / (time_end - time_start);
                    }
                    std::cout << "time_end - time_start: " << time_end - time_start << std::endl;
                    std::cout << "chunk_size: " << chunk_size << std::endl;
                    std::cout << "speed: " << speed << std::endl;

                    // std::cout << "speed: " << static_cast<double>(speed / AMMB) << "MB/s" << std::endl;
