...
private:
    ...
    EC Remote2Remote(const TransferTask &task)
    {
        ...
        struct TransferContext
        {
            ...
            bool buffer_b_ready = false;
            bool reading_finished = false;
            size_t buffer_a_len = 0; // 新增实际长度记录
            size_t buffer_b_len = 0; // 新增实际长度记录
            uint64_t offset = 0;
            explicit TransferContext(size_t buf_size)
            {
                buffer_a = new char[buf_size];
                buffer_b = new char[buf_size];
            }
            ~TransferContext()
            {
                delete[] buffer_a;
        ...
        while (ctx.offset < file_size || active_write_buf != nullptr)
        {
            ...
            // 设置需要监听的事件
            if (is_reading && !ctx.reading_finished)
            {
                FD_SET(src_session->sock, &read_fds);
                has_events = 1;
            }
            if (is_writing)
            {
                FD_SET(dst_session->sock, &write_fds);
                has_events = 1;
            }
            ...
            if (has_events)
            {
                timeval timeout{};
                timeout.tv_sec = 0;
                timeout.tv_usec = 250 * 1000; // 250ms超时

                // Windows下第一个参数可以忽略，但需要传最大值+1
                int nfds = static_cast<int>(max(src_session->sock, dst_session->sock)) + 1;
                select(nfds, &read_fds, &write_fds, nullptr, &timeout);
            }
            ...
            if (is_reading && !ctx.reading_finished && FD_ISSET(src_session->sock, &read_fds))
            {
                long rc_read = libssh2_sftp_read(srcFile, active_read_buf + read_offset,
                                                 buffer_size_ori - read_offset);
                if (rc_read > 0)
                {
                    read_offset += rc_read;
                    size_t *len_ptr = (active_read_buf == ctx.buffer_a) ? &ctx.buffer_a_len : &ctx.buffer_b_len;
                    *len_ptr = read_offset;

                    if (read_offset == buffer_size_ori || rc_read == 0)
                    {
            ...
            if ((ctx.buffer_a_ready || ctx.buffer_b_ready) && !is_writing)
            {
            ...
            if (is_writing && FD_ISSET(dst_session->sock, &write_fds))
            {
                size_t write_len = (active_write_buf == ctx.buffer_a) ? ctx.buffer_a_len : ctx.buffer_b_len;
                long rc_write = libssh2_sftp_write(dstFile, active_write_buf + write_offset,
                                                   write_len - write_offset);
                if (rc_write > 0)
                {
                    write_offset += rc_write;
                    if (write_offset == write_len)
                    {
                        ctx.offset += write_len;
                        is_writing = false;
            ...
            if (ctx.reading_finished && !is_writing)
            {
                if (ctx.buffer_a_ready || ctx.buffer_b_ready)
                {
                }
                else if (read_offset > 0)
                {
                    size_t final_len = read_offset;
