...
private:
    ...
    void _get_size(std::string path, bool exclude_dir = false, bool exclude_symlink = false, uint64_t &size)
    {
        return;
    }
...
public:
    ...
    SIZER get_size(std::string path, bool exclude_dir = false, bool exclude_symlink = false)
    {
        ...
        for (auto &item : list)
        {
            SIZER item_size = get_size(item.path, exclude_dir, exclude_symlink);
            if (std::holds_alternative<uint64_t>(item_size))
            {
            }
        }
