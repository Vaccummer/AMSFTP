...
private:
    ...
    EC Local2Remote(std::string src, std::string dst)
    {
        ...
        while (remaining > 0)
        {
            ...
            {
                pytrace(INFO, "TransferTerminate", fmt::format("Local@{}->{}@{}", src, request.nickname, dst), "Local2Remote", get_session_error_msg());
                rc_r = EC::Terminate;
                goto clean;
            }
            ...
            else if (rc == 0)
            {
                if (remaining > 0)
                {
                    pytrace(ERROR, "RemoteFileWriteError", fmt::format("Local@{}->{}@{}", src, request.nickname, dst), "Local2Remote", get_session_error_msg());
