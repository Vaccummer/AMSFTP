...
bool l_exists(std::string path)
{
    return fs::exists(fs::path(path));
}

PathType l_type(std::string path)
{
    return fs::is_regular_file(fs::path(path)) ? PathType::FILE : PathType::DIR;
}
...
public:
    ...
    TR upload(std::string src, std::string dst, TransferCallback cb_set = TransferCallback(), BufferSet buffer_set = BufferSet())
    {
