...
public:
    ...
    FileMapper(std::wstring &file_path, MapType map_type, uint64_t file_size = 0)
    {
        ...
        {
            std::cerr << "CreateFileW failed" << std::endl;
            this->file_ptr = nullptr;
            return;
        }
        ...
        if (!SetFilePointerEx(this->hFile, li, NULL, FILE_BEGIN))
        {
            ...
            FormatMessageA(
                ...
                (LPSTR)&errMsg, 0, NULL);
            std::cerr << "SetFilePointerEx failed: " << errMsg << " (Error Code: " << err << ")" << std::endl;
            LocalFree(errMsg);

            std::cerr << "SetFilePointerEx failed" << std::endl;
            this->file_ptr = nullptr;
            return;
        }
        if (!SetEndOfFile(this->hFile))
        {
            std::cerr << "SetEndOfFile failed" << std::endl;
            this->file_ptr = nullptr;
            return;
        }
