...
using WRP = std::shared_ptr<std::vector<PathInfo>>;
...
private:
    ...
    void _walk(std::string path, WRP result_ptr)
    {
        ...
        result_ptr->insert(result_ptr->end(), std::get<std::vector<PathInfo>>(list).begin(), std::get<std::vector<PathInfo>>(list).end());
        }
...
public:
    ...
    EC walk(std::string path)
    {
        WRP result_ptr = std::make_shared<std::vector<PathInfo>>();
        EC rc;
        rc = exists(path);
        if (rc != EC::PassCheck)
        {
