...
public:
    ...
    EC init()
    {
        ...
        {
            pytrace("SocketInitError", request.nickname, "WSAStartup", "Windows Sockets API initialization failed");
        ...
        {
            pytrace("SocketCreateError", request.nickname, "CreateSocket", "Create but get invalid socket");
        ...
        if (selectRet <= 0)
        {
            if (selectRet == 0)
            {
                pytrace("SocketConnectError", request.nickname, "ConnectSocket", "Connect Timeout");
            }
            else
            {
                pytrace("SocketConnectError", request.nickname, "ConnectSocket", "Connect Failed");
            }
        ...
        if (!session)
        {
            pytrace("SessionCreateError", request.nickname, "SessionInit", "Session initialization failed");
        ...
        if (rc_handshake != 0)
        {
            pytrace("SessionEstablishError", request.nickname, "SessionHandshake", "Session handshake failed");
    ...
    void pytrace(std::string error_name = "", std::string target = "", std::string action = "", std::string msg = "")
    {
        if (is_trace)
        {
            std::vector<std::string> error_info = {error_name, target, action, msg};
            trace_cb(error_info);
        }
    }
};
...
public:
    ...
    {
        if (py::isinstance<py::function>(trace_cb))
        {
    ...
    void SetSessionPyTrace(py::object trace_cb)
    {
        if (py::isinstance<py::function>(trace_cb))
        {
            this->amsession->trace_cb = py::cast<py::function>(trace_cb);
