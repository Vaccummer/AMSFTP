...
struct TransferTask
{
    ...
    std::string dst;
    uint64_t size_f = 0;
    TransferTask(std::string src, std::string dst, uint64_t size_f)
        : src(src), dst(dst), size_f(size_f) {}
};
...
private:
    ...
    EC Remote2Local(std::string src, std::string dst)
    {
        ...
        while (offset < file_size)
        {
            ...
            {
                rc_r = cast_libssh2_error(libssh2_sftp_last_error(src_client->amsession->sftp));
                goto clean;
            }
        }
...
public:
    ...
    EC _transfer_file(std::string src, std::string dst)
    {
        ...
        {
            return Local2Remote(src, dst);
        }
        else if (src_client && !dst_client)
        {
            return Remote2Local(src, dst);
        }
        else if (src_client && dst_client)
        {
            return Remote2Remote(src, dst);
        }
    }

    TR transfer(std::string src, std::string dst)
    {
        EC init_code;
        EC rc;
        init_code = init();
        ...
        if (!fs::is_directory(src_path))
        {
            ...
            current_filename = src;
            if (callback.need_filename_cb)
            {
                py::gil_scoped_acquire acquire;
                callback.filename_cb(src);
            }
            rc = _transfer_file(src, dst);
            result[dst] = rc;
            if (rc != EC::Success && callback.need_error_cb)
            {
                py::gil_scoped_acquire acquire;
                callback.error_cb(src, rc);
            }
            return result;
        }
