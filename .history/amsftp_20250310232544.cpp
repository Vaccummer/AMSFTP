...
std::string sftp_error_message(unsigned long errcode)
{
    switch (errcode)
    {
    case LIBSSH2_FX_OK:
...
public:
    ...
    EC init()
    {
        ...
        if (!request.password.empty())
        {
            ...
            for (auto &private_key : private_keys)
            {
                ...
                if (rca == 0)
                {
                    ...
                    pytrace(INFO, "AuthorizeSuccess", request.nickname, "PublicKeyAuthorize", msg);
                    rc = TransferErrorCode::Success;
                    goto OK;
                }
            }
        OK:
            ...
            if (request.trash_dir.empty())
            {
                ...
                pytrace(INFO, "TrashDirSet", "TrashDir", "DefaultTrashDir", msg);
            }
    ...
    EC check()
    {
        ...
        if (rct != 0)
        {
            EC rc;
            {
                pytrace(CRITICAL, "SftpCheckError", "TrialPath", "StatCheck", "Failed to get pathInfo");
        ...
        pytrace(INFO, "SftpCheckSuccess", "TrialPath", "StatCheck", "Success");
        return EC::Success;
    }
...
public:
    ...
    std::string sftp_error_msg()
    {
        char *errmsg = NULL;
        int errmsg_len;
        int errcode = libssh2_sftp_last_error(amsession->sftp, &errmsg, &errmsg_len, 0);
        return std::string(errmsg);
    }
