...
public:
    // 隐式转换构造函数
    WindowsPath(const fs::path &path)
    {
        m_path = normalize_separators(path.string());
    }

    // 支持各种字符串类型的构造函数
    WindowsPath(const std::string &str) : WindowsPath(fs::path(str)) {}
    WindowsPath(const char *str) : WindowsPath(fs::path(str)) {}
    ...
    {
        return WindowsPath((fs::path(m_path) / fs::path(rhs.m_path)).string());
    }
