...
private:
    ...
    EC Remote2Remote(std::string src, std::string dst, std::shared_ptr<AMSFTPWorker> another_worker)
    {
        ...
        uint64_t buffer_size = calculate_buffer_size(file_size, 32 * AMGB);
        ...
        while (all_write < file_size)
        {
            ...
            if (rbuffer != -1 && all_read < file_size)
            {
                ...
                if (ctx.bufferd[rbuffer].read == rchunk_size)
                {
                    ...
                    {
                        buffer_size = calculate_buffer_size(file_size - all_read, sprd.read_speed) > buffer_size ? buffer_size : calculate_buffer_size(file_size - all_read, sprd.read_speed);
                    }
                    sprd.read_count++;
