...
class AMSFTPClient(BaseSFTPClient, AMFS.BasePathMatch):
    ...
    def __init__(
        self,
        request: AMData.ConRequst,
        keys: list[str],
        error_num: int = 10,
        trace_cb: typing.Any = None,
        auth_cb: typing.Any = None,
    ) -> None: ...
    def chmod(
        self, path: str, mode: str | int, recursive: bool = False
    ) -> dict[str, tuple[AMEnum.ErrorCode, str]] | tuple[AMEnum.ErrorCode, str]: ...
    def copy(
    ...
    def get_path_type(
        self, path: str
    ) -> tuple[tuple[AMEnum.ErrorCode, str], AMEnum.PathType]: ...
    ...
    def iwalk(
        self, path: str, ignore_sepcial_file: bool = True
    ) -> list[AMData.PathInfo]:
        """
        Recursive walk the path, ignore path structure, just return list[PathInfo]
        """
    def listdir(
        self, path: str, max_time_ms: int = -1
    ) -> tuple[tuple[AMEnum.ErrorCode, str], list[AMData.PathInfo]]: ...
    ...
    def remove(
        self, path: str
    ) -> list[tuple[str, tuple[AMEnum.ErrorCode, str]]] | tuple[AMEnum.ErrorCode, str]:
        """
    ...
    def stat(
        self, path: str
    ) -> tuple[tuple[AMEnum.ErrorCode, str], AMData.PathInfo]: ...
    def walk(
        self, path: str, max_depth: int = -1, ignore_sepcial_file: bool = True
    ) -> tuple[tuple[AMEnum.ErrorCode, str], list[tuple[list[str], AMData.PathInfo]]]:
        """
        Recursive walk the path, record parent dir, return list[tuple[list[str],PathInfo]]
        """
    def SetPublicVar(
        self, key: str, value: typing.Any, overwrite: bool = False
    ) -> tuple[AMEnum.ErrorCode, str]:
        """
        ...
        Args:
            ...
            value: The Python object to store (will be deep copied)
            overwrite: If False, returns error if key exists; if True, overwrites existing value

        Returns:
            tuple[ErrorCode, str]: (ErrorCode.Success, "") on success, or error information
        """
    def GetPublicVar(self, key: str, default_value: typing.Any = None) -> typing.Any:
        """
        Get a stored Python object, returns a reference (allows direct modification).
        ...
        Returns:
            A reference to the stored object (shared with C++), or default_value if not found

        Note:
            - Returns a reference, not a copy
            - Modifications to the returned object will affect the stored object
            - Multiple calls return the same object (shared reference)
        """
    def DelPublicVar(self, key: str) -> bool:
        """
        Delete a stored Python object.
    ...
    def ClearPublicVar(self) -> None:
        ...
        """
    def GetAllPublicVars(self) -> dict[str, typing.Any]:
        """
        Get all stored Python objects as a dictionary with deep copies.

        Returns:
            A dictionary containing deep copies of all stored objects
        """
...
class AMSFTPWorker:
    ...
    def __init__(
        self, callback: AMData.TransferCallback = ..., cb_interval_s: float = 0.1
    ) -> None: ...
    def load_tasks(
        self,
        src: str,
        dst: str,
        hostd: AMData.HostMaintainer,
        src_hostname: str = "",
        dst_hostname: str = "",
        overwrite: bool = False,
        mkdir: bool = True,
        ignore_sepcial_file: bool = True,
    ) -> tuple[tuple[AMEnum.ErrorCode, str], list[AMData.TransferTask]]: ...
    def pause(self) -> None: ...
    def reset(self) -> None:
        """
    ...
    def transfer(
        ...
        hostd: AMData.HostMaintainer,
        chunk_large: int = 16777216,
        chunk_middle: int = 2097152,
        chunk_small: int = 262144,
    ) -> list[AMData.TransferTask]: ...
...
class BaseSFTPClient(AMSession):
    def ConductCmd(
        self, cmd: str
    ) -> tuple[tuple[AMEnum.ErrorCode, str], tuple[str, int]]:
        """
