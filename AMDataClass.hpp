...
struct ErrorCBInfo {
    ...
    std::string src_host;
    std::string dst_host;
    ErrorCBInfo(std::pair<ErrorCode, std::string> ecm, std::string src, std::string dst, std::string src_host,
                std::string dst_host)
  ...
  public:
    ...
    WindowsFileMapper(const std::string &file_path, MapType map_type, std::string &error_msg, uint64_t file_size = 0) {
        ...
        if (map_type == MapType::Read) {
            this->hFile = CreateFileW(AMFS::Str::AMStr(file_path).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    DONE:
        if (!is_ok) {
            ...
            FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, err,
...
class StreamRingBuffer {
  ...
  public:
    ...
    // 获取可读数据量
    size_t available() const { return tail.load(std::memory_order_acquire) - head.load(std::memory_order_relaxed); }
    ...
    // 提交读取消费的数据量
    void commit_read(size_t len) { head.fetch_add(len, std::memory_order_release); }
