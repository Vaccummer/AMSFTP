...
`channel export <terminal>@<channel> <local-file>` 会把该 channel 的 VT 历史以纯文本追加写入本地文件。颜色、加粗等格式会被忽略；父级目录不存在时会自动创建；相对路径会先解析为本地绝对路径，再用于写入和打印。
...
- `channel_num`、`term_num`：当前管理的 channel 和 terminal 总数
- `channel_ok`、`channel_disconnected`：当前 terminal/client 下的 channel 状态计数
- `term_ok`、`term_disconnected`：terminal 会话状态计数
- `channel_name`：当前 terminal/client 的当前 channel 名称，没有时为空字符串
