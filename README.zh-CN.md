...
当前 `CMakeLists.txt` 只会从 `src/foreign/tomlread/target/release` 查找 `rust_toml_read.lib`，所以即使你后面准备用 debug preset，这一步也应该保持 `--release`。
...
因此，当使用 `win-clang-debug` preset 时，生成的程序路径为：
...
在交互模式中，AMSFTP 会保留当前 client 或 session 上下文，并提供命令补全与高亮。通过非交互启动后输入 `bash`，也可以进入这套工作流。
