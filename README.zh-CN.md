...
1. 先以 `release` 模式构建 Rust 辅助库。

当前 `CMakeLists.txt` 只会从 `src/foreign/tomlread/target/release` 查找 `rust_toml_read.lib`，所以这一步应该保持 `--release`。该 crate 也包含交互式终端历史渲染所需的 Rust VT backend 导出。
...
term 目标语法是 `host@name` 或单独的 `name`。`ssh name` 会在当前 client 下解析 term，单独执行 `ssh` 会重新进入本次程序运行中上一次进入的 term。

终端快捷键：

- `Ctrl+]`：进入 God Mode。
- God Mode 中，`Esc` 返回当前 term，`Ctrl+]` 发送真实的 `Ctrl+]`，`Left` 或 `Shift+Tab` 切到上一个 term，`Right` 或 `Tab` 切到下一个 term，`q` detach 回到 AMSFTP prompt。
- 远端或本地 shell 正常退出时，term 会关闭。
- detach 后可以用 `term ls` 查看仍可复用的 term。

`config/settings.toml` 中的 prompt 模板还可以使用这些 Lua 全局变量：

- `term_num`：当前管理的 term 总数
- `term_ok`、`term_disconnected`：term 会话状态计数
