 ...
 */
void PrintHostDetail_(AMInterface::prompt::AMPromptIOManager &prompt, const std::string &nickname,
 ...
 */
bool PromptHostText_(AMInterface::prompt::AMPromptIOManager &prompt, const std::string &label,
 ...
 */
bool PromptHostBool_(AMInterface::prompt::AMPromptIOManager &prompt, const std::string &label,
 ...
 */
bool PromptHostProtocol_(AMInterface::prompt::AMPromptIOManager &prompt, ClientProtocol current,
                         ClientProtocol *out) {
  if (!out) {
    return false;
  }
 ...
 */
bool PromptHostInt64_(AMInterface::prompt::AMPromptIOManager &prompt, const std::string &label,
 ...
 */
void PrintHostCompact_(AMInterface::prompt::AMPromptIOManager &prompt,
 ...
 */
ECM PromptAddHostConfig_(AMInterface::prompt::AMPromptIOManager &prompt,
 ...
 */
ECM PromptEditHostConfig_(AMInterface::prompt::AMPromptIOManager &prompt,
...
ECM HostProfileGateway::ListHosts(bool detail) {
  ...
  if (!detail) {
    PrintHostCompact_(prompt_io_manager_, nicknames);
    return Ok();
  }
  if (nicknames.empty()) {
  ...
  for (const auto &nickname : nicknames) {
    ...
    PrintHostDetail_(prompt_io_manager_, nickname, entry);
    prompt_io_manager_.Print("");
  }
  return Ok();
}
...
ECM HostProfileGateway::ListPrivateKeys(bool detail) {
  ...
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    prompt_io_manager_.Print(AMStr::fmt("[{}]  {}", i, styled_path));
  }
  return Ok();
}
...
ECM HostProfileGateway::AddHost(const std::string &nickname) {
  HostConfig entry;
  ECM prompt_rcm =
      PromptAddHostConfig_(prompt_io_manager_, host_config_manager_, nickname, &entry);
  if (!isok(prompt_rcm)) {
    return prompt_rcm;
  }
...
ECM HostProfileGateway::EditHost(const std::string &nickname) {
  ...
  ECM prompt_rcm = PromptEditHostConfig_(prompt_io_manager_, nickname, &entry);
  if (!isok(prompt_rcm)) {
    return prompt_rcm;
  }
  return host_config_manager_.AddHost(entry, true);
...
ECM HostProfileGateway::EditProfile(const std::string &nickname) {
  return prompt_io_manager_.Edit(nickname);
}
...
ECM HostProfileGateway::GetProfiles(const std::vector<std::string> &nicknames) {
  return prompt_io_manager_.Get(nicknames);
}
...
ECM ConfigBackedVarRepository::Load(
  ...
  if (out_vars->find(AMDomain::var::kPublic) == out_vars->end()) {
    (*out_vars)[AMDomain::var::kPublic] = {};
  }
  return Ok();
}
 ...
 */
PromptConfigSaver::PromptConfigSaver(
 ...
 *
 * Prompt manager persists profile/history by re-binding current nickname.
 */
ECM PromptConfigSaver::SavePromptConfig(bool dump_now) {
  (void)dump_now;
  (void)prompt_profile_history_manager_.ChangeClient(
...
ECM ClientSessionGateway::StatPaths(const std::vector<std::string> &paths,
  ...
  for (const auto &raw : paths) {
    ...
    if (!client) {
      if (nickname.empty()) {
        client = client_service_.GetCurrentClient();
      } else {
        auto resolved_client = client_service_.GetClient(nickname, true);
...
TransferExecutorPort::TransferExecutorPort(
    ...
    : transfer_service_(transfer_service), prompt_io_manager_(&prompt_manager),
 ...
 */
void PrintTransferSet_(AMInterface::prompt::AMPromptIOManager &prompt,
...
void TaskGateway::PrintTaskSummary_(
  ...
  prompt_io_manager_->FmtPrint("Task {} [{}] files {}/{} size {}/{} result {} {}",
                            task_summary.id,
...
void TaskGateway::PrintTaskEntries_(
  ...
  if (task_view.entries.empty()) {
    prompt_io_manager_->FmtPrint("Task {} has no entries.", task_view.summary.id);
    return;
  }
...
void TaskGateway::PrintTaskSets_(
  ...
  if (task_view.transfer_sets.empty()) {
    prompt_io_manager_->FmtPrint("Task {} has no transfer sets.",
  ...
  for (const auto &set : task_view.transfer_sets) {
    ...
    for (const auto &src : set.srcs) {
      prompt_io_manager_->FmtPrint("    src: {}", RenderEndpoint_(src));
    }
  }
}
...
ECM TaskGateway::Thread(int num) {
  ...
  if (num < 0) {
    prompt_io_manager_->FmtPrint("Current transfer thread count: {}",
                              rcm.second.empty() ? "0" : rcm.second);
    return Ok();
  }
  prompt_io_manager_->FmtPrint("Updated transfer thread count: {}",
...
ECM TaskGateway::SubmitCachedTransferSets(bool quiet, bool is_async) {
  ...
  if (!quiet && prompt_io_manager_) {
    for (const auto &set : transfer_set_views) {
      PrintTransferSet_(*prompt_io_manager_, set);
      prompt_io_manager_->Print("");
    }
    bool canceled = false;
    if (!prompt_io_manager_->PromptYesNo("Submit cached transfer sets? (y/N): ",
                                      &canceled)) {
      prompt_io_manager_->Print("Canceled.");
  ...
  const auto confirm_policy =
      quiet ? AMApplication::TransferWorkflow::TransferConfirmPolicy::AutoApprove
            : AMApplication::TransferWorkflow::TransferConfirmPolicy::
                  RequireConfirm;
  AMApplication::TransferWorkflow::TransferAppService::WildcardConfirmFn confirm =
      BuildWildcardConfirmFn_(prompt_io_manager_, confirm_policy);
...
ECM TaskGateway::QueryCachedTransferSet(size_t index) {
  ...
  if (prompt_io_manager_) {
    PrintTransferSet_(*prompt_io_manager_, transfer_set);
  }
  return Ok();
}
