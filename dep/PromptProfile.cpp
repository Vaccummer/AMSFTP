 ...
 */
const AMApplication::config::PromptProfileSettings *FindDefaultProfileSettings_(
...
void PrintPromptProfile_(AMPromptIOManager &prompt, const std::string &name,
...
/**
 * @brief Initialize prompt profile args from typed settings with fallback
 * defaults.
 */
 ...
 */
AMApplication::config::PromptProfileSettings
AMPromptProfileArgs::ToSettings() const {
 ...
 */
AMPromptProfileHistoryManager::~AMPromptProfileHistoryManager() {
...
ECM AMPromptProfileHistoryManager::Init() {
  CollectHistory_();
 ...
 * defaults.
 */
AMPromptProfileArgs AMPromptProfileHistoryManager::BuildPromptProfileArgs_(
    const AMApplication::config::PromptProfileSettings &settings,
 ...
 */
ECM AMPromptProfileHistoryManager::ReloadPromptProfiles() {
 ...
 */
ECM AMPromptProfileAdmin::EditProfile_(const std::string &nickname) {
  profile_history_manager_.EnsurePromptProfilesLoaded_();
  ...
  {
    std::lock_guard<std::mutex> lock(profile_history_manager_.profile_mtx_);
    auto it = profile_history_manager_.prompt_profiles_.find(target);
    if (it != profile_history_manager_.prompt_profiles_.end()) {
      working = it->second;
    } else {
      auto star_it =
          profile_history_manager_.prompt_profiles_.find(kDefaultPromptProfile);
      if (star_it != profile_history_manager_.prompt_profiles_.end()) {
        working = star_it->second;
      } else {
        working = profile_history_manager_.default_prompt_profile_args_;
      }
      working.name = target;
  ...
  AMPromptIOManager &prompt = prompt_io_manager_;
...

  {
    ...
    const auto release_runtime_profile = [this](AMPromptProfileArgs &profile) {
      if (profile.ic_profile) {
        if (profile.ic_profile ==
            profile_history_manager_.core_prompt_profile_) {
          profile_history_manager_.core_prompt_profile_ = nullptr;
        }
        ic_profile_free(profile.ic_profile);
        profile.ic_profile = nullptr;
      }
      profile_history_manager_.history_seeded_clients_.erase(profile.name);
    };

    auto existing = profile_history_manager_.prompt_profiles_.find(target);
    if (existing != profile_history_manager_.prompt_profiles_.end()) {
      release_runtime_profile(existing->second);
    }
    working.ic_profile = nullptr;
    profile_history_manager_.prompt_profiles_[target] = working;
    profile_history_manager_.history_seeded_clients_.erase(target);

    if (target == kDefaultPromptProfile) {
      profile_history_manager_.default_prompt_profile_args_ = working;
      profile_history_manager_.default_prompt_profile_args_.name =
          kDefaultPromptProfile;
      profile_history_manager_.default_prompt_profile_args_.from_default =
          false;
      profile_history_manager_.default_prompt_profile_args_.ic_profile = nullptr;

      for (auto &pair : profile_history_manager_.prompt_profiles_) {
  ...
  if (target != kDefaultPromptProfile) {
    (void)profile_history_manager_.EnsureCorePromptProfileForClient_(target);
  }
  if (!profile_history_manager_.active_core_nickname_.empty()) {
    (void)profile_history_manager_.UseCorePromptProfileForClient_(
        profile_history_manager_.active_core_nickname_);
  }
  return Ok();
}
 ...
 */
ECM AMPromptProfileAdmin::Edit(const std::string &nickname) {
  const std::string target = NormalizeProfileNickname_(nickname);
  if (target.empty()) {
    return Err(EC::InvalidArg, "empty profile nickname");
  }
 ...
 */
ECM AMPromptProfileAdmin::Get(const std::vector<std::string> &nicknames) {
  if (nicknames.empty()) {
    ...
    for (const auto &target : targets) {
      ...
      if (it != profile_history_manager_.prompt_profiles_.end()) {
        ...
        if (star_it != profile_history_manager_.prompt_profiles_.end()) {
          ...
          profile = &profile_history_manager_.default_prompt_profile_args_;
        }
      }
      if (!first) {
 ...
 */
AMPromptProfileArgs &
AMPromptProfileHistoryManager::EnsurePromptProfileForClient_(
 ...
 */
AMPromptProfileArgs &
AMPromptProfileArgs &AMPromptProfileHistoryManager::ResolvePromptProfileArgs(
    const std::string &nickname) {
  EnsurePromptProfilesLoaded_();
  const std::string key = NormalizeProfileNickname_(nickname);
  std::lock_guard<std::mutex> lock(profile_mtx_);
 ...
 */
AMPromptProfileArgs *
AMPromptProfileHistoryManager::GetCurrentPromptProfileArgs() {
  EnsurePromptProfilesLoaded_();
...
AMPromptProfileHistoryManager::GetCurrentPromptProfileArgs() const {
  return const_cast<AMPromptProfileHistoryManager *>(this)
      ->GetCurrentPromptProfileArgs();
}

/**
 * @brief Ensure prompt profiles are loaded.
 */
void AMPromptProfileHistoryManager::EnsurePromptProfilesLoaded_() {
  {
    std::lock_guard<std::mutex> lock(profile_mtx_);
    if (profiles_loaded_) {
      return;
 ...
 */
void AMPromptProfileHistoryManager::CollectHistory_() {
  history_map_.clear();
 ...
 */
void AMPromptProfileHistoryManager::SetHistoryEnabled(bool enabled) {
 ...
 */
void AMPromptProfileHistoryManager::AddHistoryEntry(const std::string &line) {
  if (line.empty()) {
    return;
  }
  AMPromptProfileArgs *profile = GetCurrentPromptProfileArgs();
 ...
 */
ECM AMPromptProfileHistoryManager::ChangeClient(const std::string &nickname) {
  const std::string target = NormalizeProfileNickname_(nickname);
 ...
 */
void AMPromptProfileHistoryManager::FlushHistory() {
