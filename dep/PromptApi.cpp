 ...
 */
ic_profile_t *AMPromptProfileHistoryManager::EnsureCorePromptProfileForClient_(
 ...
 * profile.
 */
bool AMPromptProfileHistoryManager::UseCorePromptProfileForClient_(
    const std::string &nickname) {
  AMPromptProfileArgs &profile_args = EnsurePromptProfileForClient_(nickname);
...
void AMPromptIOManager::InitIsoclineConfig() {
  profile_history_manager_.EnsurePromptProfilesLoaded_();
  (void)profile_history_manager_.ChangeClient("local");
  const AMPromptProfileArgs &default_profile =
      profile_history_manager_.ResolvePromptProfileArgs("*");
