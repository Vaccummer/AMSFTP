...
#include "TransferExecutionPool.hpp"
#include "application/transfer/TaskControlClientBridge.hpp"
...
size_t TransferExecutionPool::ClampThreadCount(size_t count) {
...
bool TransferExecutionPool::IsValidThreadId(int thread_id) const {
...
bool TransferExecutionPool::IsTaskIdUsed_(const TaskId &task_id) const {
...
TransferExecutionPool::TaskId TransferExecutionPool::GenerateTaskId_() const {
...
bool TransferExecutionPool::HasPendingTasksLocked() const {
...
void TransferExecutionPool::SetTaskPool_(
...
TransferExecutionPool::GetTaskPool_(const TaskId &task_id) const {
  if (task_id.empty()) {
    return nullptr;
  }
  std::lock_guard<std::mutex> lock(task_pool_mtx_);
...
std::shared_ptr<AMApplication::TransferRuntime::ITransferClientPoolPort>
TransferExecutionPool::TakeTaskPool_(const TaskId &task_id) {
  if (task_id.empty()) {
    return nullptr;
  }
  std::lock_guard<std::mutex> lock(task_pool_mtx_);
...
void TransferExecutionPool::ReleaseTaskResources_(const TaskId &task_id) {
...
void TransferExecutionPool::CancelPendingTasksOnExit_(const std::string &reason) {
...
void TransferExecutionPool::EnsureProgressData(
...
void TransferExecutionPool::RegisterTask(const std::shared_ptr<TaskInfo> &task_info,
...
std::optional<std::pair<TransferExecutionPool::TaskId, std::shared_ptr<TaskInfo>>>
...
void TransferExecutionPool::HandleCompletedTask(
    const std::shared_ptr<TaskInfo> &task_info) {
...
void TransferExecutionPool::InnerCallback(std::shared_ptr<TaskInfo> task_info,
...
bool TransferExecutionPool::ShouldSkipTask(std::shared_ptr<TaskInfo> task_info) {
...
ssize_t TransferExecutionPool::ResolveBufferHint_(const ClientHandle &client) {
  if (!client) {
    return -1;
  }
...
ssize_t TransferExecutionPool::CalculateBufferSize(const ClientHandle &src_client,
...
std::string TransferExecutionPool::CanonicalTaskNickname_(
...
std::vector<std::string> TransferExecutionPool::CollectTaskNicknames_(
...
std::pair<TransferExecutionPool::ECM, TransferExecutionPool::TaskClientCollection>
TransferExecutionPool::AcquireTaskClients_(
    const std::shared_ptr<TaskInfo> &task_info,
    const std::shared_ptr<AMApplication::TransferRuntime::ITransferClientPoolPort> &pool) {
  if (!task_info) {
  ...
  for (const auto &nickname : same_host_nicknames) {
    ...
    if (extra_rcm.first != EC::Success || !extra_client) {
      if (extra_rcm.first == EC::Success) {
        extra_rcm = {EC::InvalidHandle,
                     AMStr::fmt("Failed to acquire extra client for {}",
                                nickname)};
      }
...
std::vector<TransferExecutionPool::ClientHandle> TransferExecutionPool::FlattenTaskClients_(
    const TaskClientCollection &clients) {
  std::vector<ClientHandle> bound;
  bound.reserve(clients.primary_clients.size() + clients.secondary_clients.size());
  std::unordered_set<const void *> seen;
  for (const auto &[nickname, client] : clients.primary_clients) {
    (void)nickname;
    if (!client) {
      continue;
    }
    const void *raw = static_cast<const void *>(client.get());
    if (seen.insert(raw).second) {
      bound.push_back(client);
    }
  }
  for (const auto &[nickname, client] : clients.secondary_clients) {
    (void)nickname;
    if (!client) {
      continue;
    }
...
TransferExecutionPool::ClientHandle TransferExecutionPool::ResolveTaskClient_(
    const TaskClientCollection &clients, const std::string &nickname,
...
void TransferExecutionPool::ExecuteTask(std::shared_ptr<TaskInfo> task_info) {
  ...
  ScopedTaskControlClientBridge interrupt_bridge(
      task_info->control_token, FlattenTaskClients_(task_clients));
  ...
  PassOffsetCheck:
    ...
    task.rcm =
        execution_engine_->TransferSignleFile(src_client, dst_client, task_info);
    if (pd.is_pause_only()) {
...
TransferExecutionPool::TransferExecutionPool()
    : execution_engine_(AMDomain::transfer::CreateDefaultTransferExecutionPort(
          [this](std::shared_ptr<TaskInfo> task_info, WkProgressData &pd,
                 bool force) {
...
TransferExecutionPool::~TransferExecutionPool() { (void)GracefulTerminate(-1); }

TransferExecutionPool::ECM TransferExecutionPool::GracefulTerminate(int timeout_ms) {
...
size_t TransferExecutionPool::ChunkSize(int64_t size) {
  if (size < 32 * AMKB) {
    return chunk_size_;
  }
...
TransferExecutionPool::GetThreadIds() const {
...
TransferExecutionPool::ECM
TransferExecutionPool::Submit(std::shared_ptr<TaskInfo> task_info) {
  if (!task_info) {
...
std::shared_ptr<TaskInfo> TransferExecutionPool::CreateTaskInfo(
...
std::optional<TaskStatus> TransferExecutionPool::GetStatus(const TaskId &id) const {
  {
...
std::shared_ptr<TaskInfo> TransferExecutionPool::GetResult(const TaskId &id,
                                                       bool remove) {
  std::lock_guard<std::mutex> lock(result_mtx_);
  auto it = results_.find(id);
  if (it == results_.end()) {
...
TransferExecutionPool::GetTask(const TaskId &id) const {
  std::lock_guard<std::mutex> lock(registry_mtx_);
  auto it = task_registry_.find(id);
  if (it != task_registry_.end()) {
...
TransferExecutionPool::ECM TransferExecutionPool::Pause(const TaskId &id,
                                                int timeout_ms) {
...
TransferExecutionPool::ECM TransferExecutionPool::Resume(const TaskId &id,
                                                 int timeout_ms) {
  auto [task_info, active] = GetTask(id);
  if (!task_info || !active) {
    return {EC::TaskNotFound, AMStr::fmt("Task not found: {}", id)};
...
std::pair<std::shared_ptr<TaskInfo>, TransferExecutionPool::ECM>
...
size_t TransferExecutionPool::PendingCount() const {
  std::lock_guard<std::mutex> lock(queue_mtx_);
...
bool TransferExecutionPool::IsConducting() const {
  std::lock_guard<std::mutex> lock(conducting_mtx_);
  return !conducting_tasks_.empty();
}

std::unordered_set<TransferExecutionPool::TaskId>
TransferExecutionPool::GetConductingIds() const {
  std::lock_guard<std::mutex> lock(conducting_mtx_);
...
std::vector<std::string> TransferExecutionPool::GetResultIds() const {
  std::lock_guard<std::mutex> lock(result_mtx_);
