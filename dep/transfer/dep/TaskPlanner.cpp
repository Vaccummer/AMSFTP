 ...
 */
std::pair<TaskPlanner::ECM, TASKS> TaskPlanner::LoadTasks(
    const std::string &src, const std::string &dst,
    const AMDomain::client::ClientHandle &src_client,
    const AMDomain::client::ClientHandle &dst_client,
    const std::string &src_host, const std::string &dst_host, bool clone,
    bool overwrite, bool mkdir, bool ignore_special_file, bool resume,
  ...
  auto [rc3, src_stat] = src_client->IOPort().stat(src, false, timeout_ms,
                                                   start_time, control_token);
  ...
  if (src_stat.type != PathType::DIR) {
    ...
    if (resume) {
      ...
      auto [dst_stat_rcm, dst_info] = dst_client->IOPort().stat(
          dstf, false, timeout_ms, start_time, control_token);
    ...
    auto [rcm4, dst_parent_info] = dst_client->IOPort().stat(
        AMPathStr::dirname(dstf), false, timeout_ms, start_time, control_token);
    ...
    if (rcm4.first == EC::Success) {
      auto [rcm5, dst_info] = dst_client->IOPort().stat(
          dstf, false, timeout_ms, start_time, control_token);
  ...
  auto [rcm6, dst_info] = dst_client->IOPort().stat(dstf, false, timeout_ms,
                                                    start_time, control_token);
  ...
  auto [rcm7, src_pack] = src_client->IOPort().iwalk(
      srcf, true, false, nullptr, timeout_ms, start_time, control_token);
  ...
  for (auto &item : src_paths) {
    ...
    auto [rcm8, dst_info2] = dst_client->IOPort().stat(
        dst_n, false, timeout_ms, start_time, control_token);
