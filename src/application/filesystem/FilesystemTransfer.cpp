...
AcquireSourceTransferClient_(FilesystemAppService *service,
  ...
  if (!service || !clients) {
    return {nullptr,
            Err(EC::InvalidArg, __func__, "<context>", "Transfer resolver deps are null")};
  }
  if (nickname.empty()) {
    return {nullptr, Err(EC::InvalidArg, __func__, "<context>", "Source nickname is empty")};
  }
...
ECMData<SourceResolveResult> FilesystemAppService::ResolveTransferSrc(
  ...
  if (!clients) {
    return {std::move(out), Err(EC::InvalidArg, __func__, "<context>",
                                "Transfer client container pointer is null")};
  }
  if (srcs.empty()) {
    return {std::move(out),
            Err(EC::InvalidArg, __func__, "<context>", "Source path list is empty")};
  }
  ...
  for (auto &src : srcs) {
    if (src.nickname.empty()) {
      const ECM rcm = Err(EC::InvalidArg, __func__, "<context>", "Source nickname is empty");
...
FilesystemAppService::BuildTransferTasks(const SourceResolveResult &src,
  ...
  if (!dst.resolved_target.client) {
    return {std::move(out),
            Err(EC::InvalidHandle, __func__, "<context>", "Destination client is null")};
  }
  if (dst.target.path.empty()) {
    return {std::move(out),
            Err(EC::InvalidArg, __func__, "<context>", "Destination path is empty")};
  }

  const std::string dst_host = dst.target.nickname;
  const auto check_stop = [&control]() -> ECM {
    if (control.IsInterrupted()) {
      return Err(EC::Terminate, __func__, "<context>", "BuildTransferTasks interrupted");
    }
    if (control.IsTimeout()) {
      return Err(EC::OperationTimeout, __func__, "<context>", "BuildTransferTasks timed out");
    }
    return OK;
  };
  ...
  if (opt.clone) {
    ...
    if (!source_data.resolved_target.client) {
      return {std::move(out),
              Err(EC::InvalidHandle, __func__, "<context>",
    ...
    if (opt.ignore_special_file && source_root.is_special()) {
      return {std::move(out),
              Err(EC::NotAFile, __func__, "<context>",
    ...
    if (dst.dst_info.has_value()) {
      ...
      if (src_is_dir != dst_is_dir) {
        return {std::move(out),
                Err(EC::NotADirectory, __func__, "<context>",
  ...
  while (!pending.empty()) {
    ...
    if (!state.src_client) {
      append_warning("", state.node.path,
                     Err(EC::InvalidHandle, __func__, "<context>", "Source client is null"));
      continue;
    }
    ...
    if (state.node.type == PathType::DIR) {
      ...
      if (list_result.data.empty()) {
        if (!opt.mkdir) {
          append_warning(
              state.node.path, mapped_dst,
              Err(EC::ParentDirectoryNotExist, __func__, "<context>",
...
ECMData<HttpDownloadPlan> FilesystemAppService::BuildHttpDownloadPlan(
  ...
  if (base_target.is_wildcard ||
    ...
    return {std::move(out),
            Err(EC::InvalidArg, __func__, "<context>", "wget destination cannot be wildcard")};
  }
  ...
  if (use_cwd_default) {
    ...
    if (base_stat.data.type != PathType::DIR) {
      return {std::move(out),
              Err(EC::NotADirectory, __func__, "<context>", AMStr::fmt("cwd is not directory: {}",
