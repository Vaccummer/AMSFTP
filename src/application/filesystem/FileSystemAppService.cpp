...
FilesystemAppService::GetClientHome(ClientHandle client,
  ...
  if (!client) {
    return {"", Err(EC::InvalidHandle, __func__, "<context>", "Client handle is null")};
  }
  ...
  if (home.empty()) {
    return {"",
            Err(EC::CommonFailure, __func__, "<context>", "Client home directory is empty")};
  }
  return {home, OK};
}
...
FilesystemAppService::GetCwd(const ClientControlComponent &control) {
  ...
  if (!client_service_) {
    return {std::move(out),
            Err(EC::InvalidHandle, __func__, "<context>", "client service is null")};
  }
  ...
  if (!client) {
    return {std::move(out),
            Err(EC::ClientNotFound, __func__, "<context>", "Current client not found")};
  }
...
ECM FilesystemAppService::EnsureClientWorkdir(
  ...
  if (!metadata_opt.has_value()) {
    return Err(EC::CommonFailure, __func__, "<context>", "Client metadata not found");
  }
  ...
  const auto try_resolve_dir =
    ...
    if (normalized_candidate.empty()) {
      return {"", Err(EC::InvalidArg, __func__, "<context>", "empty workdir candidate")};
    }
    ...
    if (absolute_path.empty()) {
      return {"", Err(EC::InvalidArg, __func__, "<context>", "invalid workdir candidate")};
    }
    ...
    if (stat_result.data.info.type != PathType::DIR) {
      return {"", Err(EC::NotADirectory, __func__, "<context>",
...
ECMData<PathTarget> FilesystemAppService::PeekCdHistory() const {
  ...
  if (list.empty()) {
    return {PathTarget{}, Err(EC::InvalidArg, __func__, "<context>", "cd history is empty")};
  }
...
ECM FilesystemAppService::ChangeDir(PathTarget path,
  ...
  auto metadata_opt = ClientAppService::GetClientMetadata(client);
  if (!metadata_opt.has_value()) {
    return {EC::CommonFailure, __func__, "<context>", "Client metadata not found"};
  }
  ClientMetaData metadata = *metadata_opt;
...
ECMData<PathEntry> FilesystemAppService::StatEntry(
  ...
  if (!(resolved.rcm) || !resolved.data.client) {
    return {PathEntry{}, (resolved.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
...
FilesystemAppService::TestRTT(const std::string &nickname,
  ...
  if (!(get_result.rcm) || !get_result.data) {
    return {-1.0, (get_result.rcm)
                      ? Err(EC::InvalidHandle, __func__, "<context>", "Client is null")
                      : get_result.rcm};
  }
...
FilesystemAppService::ResolveTrashDir(const PathTarget &source,
  ...
  if (!(source_resolved.rcm) || !source_resolved.data.client) {
    return {PathTarget{}, (source_resolved.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
                                                      "Resolved client is null")
                                                : source_resolved.rcm};
  }
  ...
  if (!metadata.has_value()) {
    return {PathTarget{},
            Err(EC::CommonFailure, __func__, "<context>", "Client metadata not found")};
  }
...
ECM FilesystemAppService::Rename(const PathTarget &src, const PathTarget &dst,
  ...
  if (!(src_resolved.rcm) || !src_resolved.data.client) {
    return (src_resolved.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
  ...
  if (!(dst_resolved.rcm) || !dst_resolved.data.client) {
    return (dst_resolved.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
  ...
  if (!dst_parent.empty()) {
    ...
    if (!(parent_stat.rcm)) {
      if (AMDomain::filesystem::services::IsPathNotExistError(
        ...
        if (!mkdir) {
          return Err(EC::ParentDirectoryNotExist, __func__, "<context>",
  ...
  if ((dst_stat.rcm)) {
    if ((dst_stat.data.info.type == PathType::DIR) != src_is_dir) {
      return Err(EC::PathAlreadyExists, __func__, "<context>",
                 AMStr::fmt("Destination exists with different type: {}",
                            resolved_dst.abs_path));
    }
    if (!overwrite) {
      return Err(
          EC::PathAlreadyExists, __func__, "<context>",
...
FilesystemAppService::PrepareRmfile(std::vector<PathTarget> targets,
  ...
  ECM status = OK;
  if (targets.empty()) {
    const ECM rcm = Err(EC::InvalidArg, __func__, "<context>", "No target is given");
  ...
  for (auto &target : targets) {
    if (control.IsInterrupted()) {
      const ECM rcm = Err(EC::Terminate, __func__, "<context>", "Interrupted by user");
      plan.rcm = rcm;
      return {std::move(plan), rcm};
    }
    if (control.IsTimeout()) {
      const ECM rcm = Err(EC::OperationTimeout, __func__, "<context>", "Operation timed out");
      plan.rcm = rcm;
      return {std::move(plan), rcm};
    }
    ...
    if (AMDomain::filesystem::services::HasWildcard(target.path)) {
      ...
      if (find_result.data.empty()) {
        AddPathError_(
            ...
            Err(EC::InvalidArg, __func__, "<context>", "Wildcard path matched no target"));
        continue;
      }
  ...
  for (const auto &target : expanded_targets) {
    ...
    if (!(resolved_result.rcm) || !resolved_result.data.client) {
      const ECM rcm = (resolved_result.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
                                                  "Resolved client is null")
                                            : resolved_result.rcm;
    ...
    if (stat_result.data.type == PathType::DIR) {
      const ECM rcm = Err(EC::NotAFile, __func__, "<context>",
  ...
  if (plan.validated_targets.empty() && (status)) {
    status = Err(EC::InvalidArg, __func__, "<context>", "No valid file target");
  }
  plan.rcm = status;
...
FilesystemAppService::ExecuteRmfile(
  ...
  if (plan.validated_targets.empty()) {
    const ECM rcm = Err(EC::InvalidArg, __func__, "<context>", "No valid file target");
  ...
  for (const auto &resolved : plan.validated_targets) {
    if (control.IsInterrupted()) {
      return {std::move(errors),
              Err(EC::Terminate, __func__, "<context>", "Interrupted by user")};
    }
    if (control.IsTimeout()) {
      return {std::move(errors),
              Err(EC::OperationTimeout, __func__, "<context>", "Operation timed out")};
    }

    PathTarget target = resolved.target;
    if (!resolved.client) {
      const ECM rcm = Err(EC::InvalidHandle, __func__, "<context>", "Resolved client is null");
...
ECMData<std::vector<std::pair<PathTarget, ECM>>> FilesystemAppService::Rmdir(
  ...
  for (auto &target : targets) {
    ...
    if (stat_result.data.info.type != PathType::DIR) {
      const ECM rcm = Err(
          EC::NotADirectory, __func__, "<context>",
...
ECMData<PermanentRemovePlan> FilesystemAppService::PreparePermanentRemove(
  ...
  for (auto &[nickname, entries] : matched_map) {
    ...
    for (const auto &entry : compacted) {
      ...
      if (!(root_resolved.rcm) || !root_resolved.data.client) {
        const ECM rcm = (root_resolved.rcm) ? Err(EC::InvalidHandle, __func__, "<context>",
                                                  "Resolved client is null")
                                            : root_resolved.rcm;
  ...
  if (plan.ordered_delete_paths.empty() && (status)) {
    status =
        Err(EC::InvalidArg, __func__, "<context>", "No valid target for permanent remove");
  }
  plan.rcm = status;
  return {std::move(plan), status};
}
...
FilesystemAppService::ExecutePermanentRemove(
  ...
  if (plan.ordered_delete_paths.empty()) {
    return {std::move(errors), Err(EC::InvalidArg, __func__, "<context>",
...
FilesystemAppService::Saferm(std::vector<PathTarget> targets,
  ...
  if (compacted_targets.empty()) {
    return {std::move(errors),
            (status) ? Err(EC::InvalidArg, __func__, "<context>", "No valid target for saferm")
                     : status};
  }
  ...
  for (const auto &source : compacted_targets) {
    ...
    if (bucket_state_it == bucket_rcm_map.end()) {
      ...
      if (!(trash_result.rcm)) {
        ...
        if (!(trash_stat.rcm)) {
          ...
          prepare_rcm = Err(EC::NotADirectory, __func__, "<context>",
    ...
    if (bucket_it == bucket_dir_map.end()) {
      AddSafermError_(
          ...
          Err(EC::CommonFailure, __func__, "<context>", "Missing prepared trash bucket"));
      continue;
    }
    ...
    if (!found_unique) {
      AddSafermError_(&errors, &status, source,
                      Err(EC::CommonFailure, __func__, "<context>",
