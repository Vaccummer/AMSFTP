...
ECMData<std::vector<PathInfo>> FilesystemAppService::find(
  ...
  auto stat_result = client->IOPort().stat({literal_root, false}, control);
  ...
  push_state(Cursor{stat_result.info, 0});
  ...
  while (!pending.empty()) {
    ...
    if (seg.kind == DoubleStar) {
      ...
      if (cache_it != dir_cache.end()) {
        ...
        auto list_result = client->IOPort().listdir({cur.node.path}, control);
        ...
        auto insert_it =
            dir_cache.emplace(cur.node.path, std::move(list_result.entries))
                .first;
