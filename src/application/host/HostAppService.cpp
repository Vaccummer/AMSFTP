...
ECM HostAppService::FlushTo(
  ...
  if (!config_service->Write<HostConfigArg>(GetInitArg())) {
    return Err(EC::ConfigDumpFailed, __func__, "<context>", "failed to flush host config");
  }
  return OK;
}
...
ECMData<HostConfig> HostAppService::GetClientConfig(const std::string &nickname,
  ...
  if (key.empty()) {
    return {HostConfig{}, Err(EC::HostConfigNotFound, __func__, "<context>", "host not found: ")};
  }
  ...
  if (!case_sensitive) {
    ...
    if (matched.size() > 1) {
      return {HostConfig{},
              Err(EC::HostConfigNotFound, __func__, "<context>", AMStr::fmt("ambiguous host nickname [{}], candidates: {}",
                             key, AMStr::join(matched, ", ")))};
    }
  }

  return {HostConfig{}, Err(EC::HostConfigNotFound, __func__, "<context>", AMStr::fmt("host not found: {}", normalized_key))};
}
...
HostAppService::CheckNicknameAvailable(const std::string &nickname) const {
  ...
  if (normalized.empty() || !ValidateNickname(normalized)) {
    return {"", Err(EC::InvalidArg, __func__, "<context>", "invalid host nickname")};
  }
  if (IsLocalNickname(normalized)) {
    return {"", Err(EC::InvalidArg, __func__, "<context>", "Nickname 'local' is reserved")};
  }
  if (host_configs_.find(normalized) != host_configs_.end()) {
    return {"", Err(EC::KeyAlreadyExists, __func__, "<context>", AMStr::fmt("host already exists: {}", normalized))};
  }
...
ECM HostAppService::AddHost(const HostConfig &entry, bool overwrite) {
  ...
  if (it != host_configs_.end() && !overwrite) {
    return Err(EC::KeyAlreadyExists, __func__, "<context>", AMStr::fmt("host already exists: {}",
...
ECM HostAppService::DelHost(const std::string &nickname) {
  const std::string key = NormalizeNickname(nickname);
  if (IsLocalNickname(key)) {
    return Err(EC::InvalidArg, __func__, "<context>", "local host cannot be removed");
  }
  auto it = host_configs_.find(key);
  if (it == host_configs_.end()) {
    return Err(EC::HostConfigNotFound, __func__, "<context>", AMStr::fmt("host not found: {}", key));
  }
...
ECM KnownHostsAppService::FlushTo(
  ...
  if (!config_service->Write<KnownHostEntryArg>(snapshot)) {
    return Err(EC::ConfigDumpFailed, __func__, "<context>", "failed to flush known-hosts config");
  }
  return OK;
}
...
ECM KnownHostsAppService::FindKnownHost(KnownHostQuery &query) const {
  ...
  if (it == known_hosts_.end()) {
    return Err(EC::HostConfigNotFound, __func__, "<context>", "known host entry not found");
  }
  query = it->second;
  return OK;
}
...
ECM KnownHostsAppService::UpsertKnownHost(const KnownHostQuery &query,
  ...
  if (!overwrite && known_hosts_.find(key) != known_hosts_.end()) {
    return Err(EC::KeyAlreadyExists, __func__, "<context>", "known host entry already exists");
  }
