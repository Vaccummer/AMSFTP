...
ECMData<HostConfig> HostAppService::GetClientConfig(const std::string &nickname,
  ...
  if (!case_sensitive) {
    ...
    if (matched.size() > 1) {
      return {HostConfig{},
              Err(EC::HostConfigNotFound, "", "", AMStr::fmt("ambiguous host nickname [{}], candidates: {}",
                             key, AMStr::join(matched, ", ")))};
    }
  }

  return {HostConfig{}, Err(EC::HostConfigNotFound, "", "", AMStr::fmt("host not found: {}", normalized_key))};
}
...
HostAppService::CheckNicknameAvailable(const std::string &nickname) const {
  ...
  if (host_configs_.find(normalized) != host_configs_.end()) {
    return {"", Err(EC::KeyAlreadyExists, "", "", AMStr::fmt("host already exists: {}", normalized))};
  }
