...
/// \{
///
/// Isocline keeps one active profile at a time. Existing `ic_*` APIs operate on
/// the active profile. Profiles are independent, but interactive readline
/// should still be used one prompt at a time on a single terminal.
///
...
///
/// Get the current active profile (lazily creates the default profile if
/// needed).
...
/** Extended readline entry that accepts optional initial text.
...
///   are the `b`, `i`, `u`, `r`, and `s` styles for bold, italic, underline,
///   reverse, and strike.
///
...
/** Disable or enable automatic sorting of completion items (enabled by
   default). Returns the previous setting. */
bool ic_enable_completion_sort(bool enable);

/** Set the maximum number of completion items to generate/show.
    Pass a value <= 0 to reset to the default (1000). Values above 1000 are
   clamped. Returns the previous setting. */
...
/** Set a fixed line prefix that is displayed after line breaks in multiline
   input. The prefix supports bbcode formatting and is not inserted into the
   input. Pass NULL to disable the prefix. */
...
/// While waiting, typing input cancels the pending search for the old line
/// state.
...
//--------------------------------------------------------------
/// \defgroup profile_mutators Profile-Explicit Mutators
/// Mutator variants that operate on an explicit profile.
/// Passing NULL uses the current profile.
/// \{

void ic_style_def_p(ic_profile_t *profile, const char *style_name,
                    const char *fmt);
void ic_style_open_p(ic_profile_t *profile, const char *fmt);
void ic_style_close_p(ic_profile_t *profile);

void ic_set_history_p(ic_profile_t *profile, const char *fname,
                      long max_entries);
void ic_history_remove_last_p(ic_profile_t *profile);
void ic_history_clear_p(ic_profile_t *profile);
void ic_history_add_p(ic_profile_t *profile, const char *entry);
...
void ic_set_prompt_marker_p(ic_profile_t *profile, const char *prompt_marker,
                            const char *continuation_prompt_marker);
bool ic_enable_multiline_p(ic_profile_t *profile, bool enable);
bool ic_enable_beep_p(ic_profile_t *profile, bool enable);
bool ic_enable_color_p(ic_profile_t *profile, bool enable);
bool ic_enable_history_duplicates_p(ic_profile_t *profile, bool enable);
bool ic_enable_auto_tab_p(ic_profile_t *profile, bool enable);
bool ic_enable_completion_preview_p(ic_profile_t *profile, bool enable);
bool ic_enable_completion_sort_p(ic_profile_t *profile, bool enable);
long ic_set_completion_max_items_p(ic_profile_t *profile, long max_items);
long ic_set_completion_max_columns_p(ic_profile_t *profile, long max_columns);
long ic_set_completion_max_rows_p(ic_profile_t *profile, long max_rows);
bool ic_enable_completion_number_pick_p(ic_profile_t *profile, bool enable);
bool ic_enable_completion_auto_fill_p(ic_profile_t *profile, bool enable);
void ic_set_completion_select_sign_p(ic_profile_t *profile, const char *sign);
void ic_set_completion_page_marker_p(ic_profile_t *profile, const char *marker);
bool ic_enable_multiline_indent_p(ic_profile_t *profile, bool enable);
void ic_set_line_prefix_p(ic_profile_t *profile, const char *prefix);
void ic_set_history_search_prompt_p(ic_profile_t *profile,
                                    const char *prompt_text);
bool ic_enable_inline_help_p(ic_profile_t *profile, bool enable);
bool ic_enable_hint_p(ic_profile_t *profile, bool enable);
long ic_set_hint_delay_p(ic_profile_t *profile, long delay_ms);
long ic_set_hint_search_delay_p(ic_profile_t *profile, long delay_ms);
void ic_set_tty_esc_delay_p(ic_profile_t *profile, long initial_delay_ms,
                            long followup_delay_ms);
bool ic_enable_highlight_p(ic_profile_t *profile, bool enable);
long ic_set_highlight_delay_p(ic_profile_t *profile, long delay_ms);
bool ic_enable_brace_matching_p(ic_profile_t *profile, bool enable);
void ic_set_matching_braces_p(ic_profile_t *profile, const char *brace_pairs);
bool ic_enable_brace_insertion_p(ic_profile_t *profile, bool enable);
void ic_set_insertion_braces_p(ic_profile_t *profile, const char *brace_pairs);
...
void ic_init_custom_malloc(ic_malloc_fun_t *_malloc, ic_realloc_fun_t *_realloc,
                           ic_free_fun_t *_free);
