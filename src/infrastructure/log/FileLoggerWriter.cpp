...
ECM AMInfraFileLoggerWriter::SetPath(const std::filesystem::path &path) {
  if (path.empty()) {
    ECM invalid = Err(EC::InvalidArg, __func__, "", "Logger writer path cannot be empty");
...
ECM AMInfraFileLoggerWriter::Write(const std::string &line) {
  ...
  if (stream_.bad() || stream_.fail()) {
    ECM write_rcm = Err(EC::LocalFileError, __func__, "", AMStr::fmt("Failed to write log file {}",
...
ECM AMInfraFileLoggerWriter::EnsureStreamOpen_() {
  ...
  if (path_.empty()) {
    return Err(EC::InvalidArg, __func__, "", "Logger writer path is not configured");
  }
  std::error_code ec;
  ...
  if (!parent_path.empty()) {
    ...
    if (ec) {
      return Err(EC::LocalFileError, __func__, "", AMStr::fmt("Failed to create log directory {}: {}",
  ...
  if (!stream_.is_open()) {
    return Err(EC::LocalFileError, __func__, "", AMStr::fmt("Failed to open log file {}", path_.string()));
  }
  return OK;
}
