...
private:
  ...
  void SyncCacheFromRuntimeUnlocked_() {
    ...
    for (const auto &entry : channels_) {
      ECM state = OK;
      if (!entry.second) {
        state = Err(EC::InvalidHandle, "terminal.channel.state", entry.first,
                    "Channel handle is null");
      } else {
        const auto ch_state = entry.second->GetState();
        if (ch_state.closed) {
          state = !(ch_state.last_error)
                      ? ch_state.last_error
                      : Err(EC::NoConnection, "terminal.channel.state",
                            entry.first, "Channel is closed");
        }
      }
  ...
  EnsureSessionReady_(const AMDomain::client::ClientControlComponent &control) {
    if (control.IsInterrupted()) {
      return Err(EC::Terminate, "terminal.session", request_.hostname,
                 "Interrupted by user");
    }
    if (control.IsTimeout()) {
      return Err(EC::OperationTimeout, "terminal.session", request_.hostname,
                 "Operation timed out");
    }
    if (!sftp_core_) {
      return Err(EC::NoSession, "terminal.session", request_.hostname,
                 "SFTP IO core is unavailable");
    }
    if (!sftp_core_->session) {
      return Err(EC::NoConnection, "terminal.session", request_.hostname,
...
public:
  ...
  ~SSHTerminalPort() override {
    ...
    CloseAllChannels_(true, close_control);
    std::lock_guard<std::recursive_mutex> lock(mutex_);
    SyncCacheFromRuntimeUnlocked_();
  }
  ...
  ECMData<AMT::ChannelOpenResult> OpenChannel(
    ...
    auto port = std::make_shared<RealtimeSSHChannelPort>(
        terminal_key, channel_name, sftp_core_, request_.hostname);
    if (!port) {
  ...
  ECMData<AMT::ChannelRenameResult> RenameChannel(
    ...
    if (src_it == channels_.end() || !src_it->second) {
    ...
    channels_[dst_name] = src_it->second;
  ...
  ECMData<AMT::CheckSessionResult> CheckSession(
    ...
    if (current_channel_.has_value()) {
      ...
      out.data.is_open = (it != channels_.end() && it->second &&
