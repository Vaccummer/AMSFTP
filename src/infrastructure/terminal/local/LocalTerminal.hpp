...
class LocalTerminalPort final : public AMT::ITerminalPort,
                                public ILocalTerminalChannelOwner {
private:
  ...
  static const ConPTYApi_ &ConPTYApiInstance_() {
  ...
  static bool DrainWindowsOutput_(ChannelRuntime_ *ctx, std::string *output,
    ...
    while (output->size() < max_bytes) {
      ...
      DWORD nread = 0;
      if (ReadFile(ctx->output_read, buffer.data(), want, &nread, nullptr) ==
          0) {
        ctx->eof = true;
        break;
      }
  ...
  static bool WaitReadableWindows_(ChannelRuntime_ *ctx,
  ...
  static bool WaitReadablePosix_(ChannelRuntime_ *ctx,
                                 const ControlComponent &control,
                                 bool *timed_out) {
    if (timed_out != nullptr) {
      *timed_out = false;
  ...
  static TerminalWindowInfo_ OpenWindow_(const AMT::ChannelOpenArgs &open_args,
  ...
  std::string ResolveTargetChannelName_(
  ...
  ECM ValidateSessionReady_(const ControlComponent &control,
                            const char *action) const {
    if (control.IsInterrupted()) {
      return Err(EC::Terminate, action, request_.nickname,
                 "Interrupted by user");
    }
    if (control.IsTimeout()) {
  ...
  void CloseChannelRuntime_(ChannelRuntime_ *ctx, bool force) {
    ...
    if (!ctx->process_exited && ctx->process_handle != INVALID_HANDLE_VALUE) {
      ...
      DWORD wait_rc =
          WaitForSingleObject(ctx->process_handle, force ? 10 : 300);
      if (wait_rc == WAIT_TIMEOUT) {
  ...
  AMDomain::filesystem::CheckResult ReadSessionStateFromClient_() const {
    if (!owner_client_) {
...
public:
  ...
  ConRequest GetRequest() const { return request_; }
  ...
  ReadChannel(const AMT::ChannelReadArgs &read_args,
    ...
    const std::string target_name =
        ResolveTargetChannelName_(bound_channel_name);
  ...
  WriteChannel(const AMT::ChannelWriteArgs &write_args,
    ...
    while (out.data.bytes_written < write_args.input.size()) {
      ssize_t const rc = write(
          ctx->master_fd, write_args.input.data() + out.data.bytes_written,
      ...
      out.rcm =
          Err(fec(std::error_code(errno, std::generic_category())),
  ...
  CloseChannel(const AMT::ChannelCloseArgs &close_args,
    ...
    out.data.closed = !channel_runtimes_.contains(target_name);
    out.rcm = OK;
    return out;
  }
  ...
  ListChannels(const AMT::ChannelListArgs &list_args,
    ...
    CacheSyncGuard_ cache_sync{this};
    for (const auto &entry : channel_runtimes_) {
      auto port_it = channels_.find(entry.first);
      if (port_it != channels_.end() && port_it->second) {
        out.data.channels[entry.first] = port_it->second;
        continue;
      }

      const std::string terminal_key =
          AMDomain::host::HostService::NormalizeNickname(request_.nickname);
      auto port = std::make_shared<LocalChannelPort>(terminal_key, entry.first,
                                                     this);
      if (!port) {
        continue;
      }
      channels_[entry.first] = port;
      out.data.channels[entry.first] = std::move(port);
    }
  ...
  AMDomain::filesystem::CheckResult GetSessionState() const {
    auto guard =
        const_cast<AMAtomic<TerminalStateCache_> &>(attr_cache_).lock();
    return (*guard).session_state;
  }

  std::optional<ECM> GetChannelState(const std::string &channel_name) const {
    auto guard =
        const_cast<AMAtomic<TerminalStateCache_> &>(attr_cache_).lock();
