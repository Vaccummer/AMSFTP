...
ECM IsoclineProfileManager::ChangeClient(const std::string &nickname) {
  ...
  if (!profile || !profile->IsValid() || !profile->Use()) {
    ...
    profile = BuildProfile_(active_nickname, profile_args, style_arg,
                            history_records);
    if (!profile || !profile->IsValid() || !profile->Use()) {
      return Err(EC::UnknownError, __func__, "",
...
ECM PromptIOManager::EditProfile_(const std::string &nickname) {
  ...
  if (target.empty()) {
    return Err(EC::InvalidArg, __func__, "", "empty profile nickname");
  }
  ...
  auto prompt_bool = [this, &bool_literals](const std::string &label,
    ...
    while (true) {
      ...
      ErrorFormat(ECM{EC::InvalidArg, __func__, "", "value must be true or false"});
    }
  };
  ...
  auto prompt_int64 = [this](const std::string &label, int64_t min_value,
    ...
    while (true) {
      ...
      if (!TryParseInt64_(out, &parsed)) {
        ErrorFormat(ECM{EC::InvalidArg, __func__, "", "invalid integer value"});
        continue;
      }
      if (parsed < min_value || parsed > max_value) {
        ErrorFormat(
            ECM{EC::InvalidArg, __func__, "", AMStr::fmt("value out of range [{}, {}]",
                                           min_value, max_value)});
        continue;
      }
      *value = parsed;
  ...
  if (!prompt_string("Prompt.marker: ", &working.prompt.marker) ||
    ...
    return Err(EC::ConfigCanceled, __func__, "", "profile edit canceled");
  }
  ...
  if (!AMInterface::ApplicationAdapters::Runtime::ConfigServiceOrThrow().Write(
          profile_document)) {
    return Err(EC::CommonFailure, __func__, "", "failed to update PromptProfile");
  }
  ECM dump_rcm =
...
ECM PromptIOManager::Edit(const std::string &nickname) {
  ...
  if (target == kDefaultPromptProfile) {
    return Err(EC::InvalidArg, __func__, "", "profile nickname must be a host
nickname");
  }
  if (!AMInterface::ApplicationAdapters::Runtime::HostConfigManagerOrThrow()
           .HostExists(target)) {
    return Err(EC::HostConfigNotFound, __func__, "", AMStr::fmt("host nickname not
found: {}", target));
  }
...
ECM PromptIOManager::Get(const std::vector<std::string> &nicknames) {
  if (nicknames.empty()) {
    return Err(EC::InvalidArg, __func__, "", "profile get requires at least one
nickname");
  }
