...
FileSystemCliAdapter::FileSystemCliAdapter(
    ...
    : client_service_(client_service), prompt_manager_(prompt_manager) {}
...
ECM FileSystemCliAdapter::Tree(const std::string &path, int max_depth,
  ...
  for (const auto &item : items) {
    ...
    prompt_manager_.Print(std::string(depth * 2, ' ') + item.name);
  }
  return status;
}
...
ECM FileSystemCliAdapter::TestRtt(int times,
  ...
  prompt_manager_.FmtPrint("RTT: {} ms", oss.str());
  return Ok();
}
