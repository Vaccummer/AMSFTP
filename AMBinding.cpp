...
PYBIND11_MODULE(AMSFTP, m)
{
    ...
    py::class_<AMSession, std::shared_ptr<AMSession>, AMTracer>(m, "AMSession", "Session Class")
        .def(py::init<ConRequst, std::vector<std::string>, unsigned int, py::object, py::object>(),
             py::arg("request"),
             py::arg("keys"),
             py::arg("error_num") = 10,
    ...
    py::class_<AMSFTPClient, std::shared_ptr<AMSFTPClient>, BaseSFTPClient, AMFS::BasePathMatch>(m, "AMSFTPClient", "Core SFTP Client Class")
        .def(py::init<ConRequst, std::vector<std::string>, unsigned int, py::object, py::object>(), py::arg("request"),
             ...
             py::arg("trace_cb") = py::none(),
             py::arg("auth_cb") = py::none())
        .def("SetPublicVar", &AMSFTPClient::SetPublicVar, py::arg("key"), py::arg("value"), py::arg("overwrite") = false, "Store a Python object in C++ side with deep copy, C++ owns the copy")
        .def("GetPublicVar", &AMSFTPClient::GetPublicVar, py::arg("key"), py::arg("default") = py::none(), "Get a stored Python object, returns a deep copy")
        .def("DelPublicVar", &AMSFTPClient::DelPublicVar, py::arg("key"), "Delete a stored Python object, returns True if deleted")
        .def("HasPublicVar", &AMSFTPClient::HasPublicVar, py::arg("key"), "Check if a key exists in the public variable dictionary")
        .def("ClearPublicVar", &AMSFTPClient::ClearPublicVar, "Clear all stored Python objects")
        .def("GetAllPublicVars", &AMSFTPClient::GetAllPublicVars, "Get all stored Python objects as a dictionary with deep copies")
        ...
        .def("SetPublicVar", &AMSFTPClient::SetPublicVar, py::arg("key"), py::arg("value"), py::arg("overwrite") = false, "Store a Python object in C++ side with deep copy, C++ owns the copy")
        .def("GetPublicVar", &AMSFTPClient::GetPublicVar, py::arg("key"), py::arg("default_value") = py::none(), "Get a stored Python object, returns a deep copy")
        .def("DelPublicVar", &AMSFTPClient::DelPublicVar, py::arg("key"), "Delete a stored Python object, returns True if deleted")
        .def("HasPublicVar", &AMSFTPClient::HasPublicVar, py::arg("key"), "Check if a key exists in the public variable dictionary")
        .def("ClearPublicVar", &AMSFTPClient::ClearPublicVar, "Clear all stored Python objects")
        .def("GetAllPublicVars", &AMSFTPClient::GetAllPublicVars, "Get all stored Python objects as a dictionary with deep copies");

    py::class_<HostMaintainer, std::shared_ptr<HostMaintainer>>(data, "HostMaintainer", "The Client Maintainer Class, Check clients status all the time")
        .def(py::init<int>(), py::arg("heartbeat_interval_s"))
        .def("add_host", &HostMaintainer::add_host, py::arg("nickname"), py::arg("client"), py::arg("overwrite") = false)
        ...
        .def("test_host", &HostMaintainer::test_host, py::arg("nickname"), py::arg("update") = false, "Test the host connection, return (ErrorCode, str)");
