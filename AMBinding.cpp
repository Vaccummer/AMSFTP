...
PYBIND11_MODULE(AMSFTP, m) {
    ...
    py::enum_<ErrorCode>(em, "ErrorCode", "Uniform Error Code for the whole module")
        ...
        .value("DeepcopyFunctionNotAvailable", ErrorCode::DeepcopyFunctionNotAvailable)
    ...
    py::class_<ConRequst, std::shared_ptr<ConRequst>>(data, "ConRequst", "Connection Request DataClass")
        .def(
            py::init<std::string, std::string, std::string, int, std::string, std::string, bool, size_t, std::string>(),
            py::arg("nickname"), py::arg("hostname"), py::arg("username"), py::arg("port"), py::arg("password") = "",
            py::arg("keyfile") = "", py::arg("compression") = false, py::arg("timeout_s") = 3,
            py::arg("trash_dir") = "")
    ...
    py::class_<TransferCallback, std::shared_ptr<TransferCallback>>(data, "TransferCallback",
                                                                    "Callback function Gather")
        .def(py::init<py::object, py::object, py::object>(), py::arg("total_size") = py::none(),
             py::arg("error") = py::none(), py::arg("progress") = py::none())
        .def("SetErrorCB", &TransferCallback::SetErrorCB, py::arg("error") = py::none(),
             "callable[[ErrorCBInfo], None]")
        .def("SetProgressCB", &TransferCallback::SetProgressCB, py::arg("progress") = py::none(),
             "callable[[ProgressCBInfo], Optional[TransferControl]]")
        .def("SetTotalSizeCB", &TransferCallback::SetTotalSizeCB, py::arg("total_size") = py::none(),
             "callable[[int], None]");

    py::class_<ProgressCBInfo, std::shared_ptr<ProgressCBInfo>>(data, "ProgressCBInfo",
                                                                "Progress Callback Info DataClass")
        .def(py::init<std::string, std::string, std::string, std::string, uint64_t, uint64_t, uint64_t, uint64_t>(),
             py::arg("src"), py::arg("dst"), py::arg("src_host"), py::arg("dst_host"), py::arg("this_size"),
             py::arg("file_size"), py::arg("accumulated_size"), py::arg("total_size"))
    ...
    py::class_<ErrorCBInfo, std::shared_ptr<ErrorCBInfo>>(data, "ErrorCBInfo")
        .def(py::init<ECM, std::string, std::string, std::string, std::string>(), py::arg("ecm"), py::arg("src"),
             py::arg("dst"), py::arg("src_host"), py::arg("dst_host"))
    ...
    py::class_<TransferTask, std::shared_ptr<TransferTask>>(data, "TransferTask")
        .def(py::init<std::string, std::string, std::string, std::string, uint64_t, PathType, bool>(), py::arg("src"),
             py::arg("src_host"), py::arg("dst"), py::arg("dst_host"), py::arg("size"),
    ...
    py::class_<TraceInfo, std::shared_ptr<TraceInfo>>(data, "TraceInfo", "Trace Information DataClass")
        .def(py::init<TraceLevel, ErrorCode, std::string, std::string, std::string, std::string>(), py::arg("level"),
             py::arg("error_code"), py::arg("nickname"), py::arg("target"), py::arg("action"), py::arg("message"))
    ...
    py::class_<PathInfo, std::shared_ptr<PathInfo>>(data, "PathInfo", "Path Information DataClass")
        .def(py::init<std::string, std::string, std::string, std::string, uint64_t, double, double, double, PathType,
                      uint64_t, std::string>(),
             py::arg("name"), py::arg("path"), py::arg("dir"), py::arg("owner"), py::arg("size"),
             py::arg("create_time"), py::arg("access_time"), py::arg("modify_time"),
             py::arg("path_type") = PathType::FILE, py::arg("mode_int") = 0777, py::arg("mode_str") = "rwxrwxrwx")
    ...
    py::class_<AMTracer, std::shared_ptr<AMTracer>>(data, "AMTracer", "Trace Buffer Class")
        .def(py::init<unsigned int, py::object, std::string>(), py::arg("buffer_capacity") = 10,
             py::arg("trace_cb") = py::none(), py::arg("nickname") = "")
    ...
    py::class_<AMFS::PathMatch, std::shared_ptr<AMFS::PathMatch>, AMFS::BasePathMatch>(fs, "PathMatch",
                                                                                       "Path Match Class")
        .def(py::init<>());

    py::class_<AMSFTPClient, std::shared_ptr<AMSFTPClient>, BaseSFTPClient, AMFS::BasePathMatch>(
        m, "AMSFTPClient", "Core SFTP Client Class")
        .def(py::init<ConRequst, std::vector<std::string>, unsigned int, py::object, py::object>(), py::arg("request"),
             py::arg("keys"), py::arg("error_num") = 10, py::arg("trace_cb") = py::none(),
             py::arg("auth_cb") = py::none())
        ...
        .def("GetPublicVar", &AMSFTPClient::GetPublicVar, py::arg("key"), py::arg("default") = py::none(),
        ...
        .def("SetPublicVar", &AMSFTPClient::SetPublicVar, py::arg("key"), py::arg("value"),
             py::arg("overwrite") = false, "Store a Python object in C++ side with deep copy, C++ owns the copy")
        .def("GetPublicVar", &AMSFTPClient::GetPublicVar, py::arg("key"), py::arg("default_value") = py::none(),
             "Get a stored Python object, returns a deep copy")
        .def("DelPublicVar", &AMSFTPClient::DelPublicVar, py::arg("key"),
             "Delete a stored Python object, returns True if deleted")
        .def("HasPublicVar", &AMSFTPClient::HasPublicVar, py::arg("key"),
    ...
    py::class_<HostMaintainer, std::shared_ptr<HostMaintainer>>(
        ...
        .def("test_host", &HostMaintainer::test_host, py::arg("nickname"), py::arg("update") = false,
    ...
    py::class_<AMSFTPWorker, std::shared_ptr<AMSFTPWorker>>(m, "AMSFTPWorker", "A worker for transferring files")
        .def(py::init<TransferCallback, float>(), py::arg("callback") = TransferCallback(),
        ...
        .def("load_tasks", &AMSFTPWorker::load_tasks, py::arg("src"), py::arg("dst"), py::arg("hostd"),
             py::arg("src_hostname") = "", py::arg("dst_hostname") = "", py::arg("overwrite") = false,
             py::arg("mkdir") = true, py::arg("ignore_sepcial_file") = true)
        .def("transfer", &AMSFTPWorker::transfer, py::arg("tasks"), py::arg("hostd"),
             py::arg("chunk_large") = 16 * AMMB, py::arg("chunk_middle") = 2 * AMMB,
    ...
    fs.def("IsAbs", &AMFS::IsAbs, py::arg("path"), py::arg("sep") = "")
        ...
        .def("abspath", &AMFS::abspath, py::arg("path"), py::arg("parsing_home") = true, py::arg("home") = "",
