...
class Str {
public:
    static std::wstring AMStr(const std::string& str) { return boost::locale::conv::utf_to_utf<wchar_t>(str); }

    static std::string AMStr(const std::wstring& wstr) { return boost::locale::conv::utf_to_utf<char>(wstr); }

    static std::string AMStr(wchar_t* wstr) { return boost::locale::conv::utf_to_utf<char>(wstr); }

    static std::wstring AMStr(char* str) { return boost::locale::conv::utf_to_utf<wchar_t>(str); }
    ...
    static std::pair<bool, int> endswith(const std::string& str, const std::string& suffix) {
        ...
        return std::make_pair(str.compare(str.size() - suffix.size(), suffix.size(), suffix) == 0,
...
inline std::string GetFileOwner(const std::wstring& path) {
    ...
    DWORD dwRtnCode = GetNamedSecurityInfoW(path.c_str(), SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION, &pSidOwner, NULL,
                                            NULL, NULL, &pSD);
...
inline std::tuple<double, double, double> GetTime(const std::wstring& path) {
    ...
    FindClose(hFind);
    return std::make_tuple(FileTimeToUnixTime(find_data.ftCreationTime), FileTimeToUnixTime(find_data.ftLastWriteTime),
...
public:
    ...
    PathInfo(std::string name, std::string path, std::string dir, std::string owner, uint64_t size, double create_time,
             double access_time, double modify_time, PathType type, uint64_t mode_int, std::string mode_str)
        : name(name),
          path(path),
          dir(dir),
          owner(owner),
...
inline bool IsAbs(const std::string& path, const std::string& sep = "") {
    return std::regex_search(UnifyPathSep(path, sep),
                             std::regex("^(?:[a-zA-Z]:$)|(?:~$)|(?:[A-Za-z]:[/\\\\]|/|[\\\\/]{2}|~[\\\\/])"));
}
...
// 解析真实存在的路径，返回绝对路径
// std::string realpath(const std::string &path, bool force_parsing = false, const std::string &cwd = "", const
// std::string &sep = "")
// {
...
inline std::string abspath(const std::string& path, const bool parsing_home = true, const std::string& home = "",
...
// inline void search(std::vector<std::string> &results, fs::path root, std::string name, std::vector<std::string>
// remains, SearchType type, bool use_regex, bool silence, CB callback)
// {
//     if (!remains.empty())
//     {
...
// inline std::vector<std::string> find(const std::string &path_f, SearchType type = SearchType::All, bool use_regex =
// false, bool silence = false, CB callback = nullptr)
// {
...
private:
    ...
    std::vector<PathInfo> ilistdir(const std::string& path) override { return listdir(path); }
