...
inline std::vector<PathInfo> listdir(const std::string &path,
  ...
  double time_now = timenow();
  try {
    for (const auto &entry : fs::directory_iterator(p)) {
      if (max_time_s > 0 && timenow() - time_now > max_time_s) {
        break;
      }
...
inline void _walk(std::vector<std::string> parts, WRD &result, int cur_depth,
                  int max_depth, bool ignore_sepcial_file) {
...
inline WRD walk(const std::string &path, int max_depth,
                bool ignore_sepcial_file) {
  WRD result = {};
  auto [error, info] = stat(path);
  if (!error.empty()) {
