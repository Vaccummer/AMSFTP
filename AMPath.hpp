...
public:
  ...
  static std::pair<bool, int> endswith(const std::string &str,
  ...
  isPatternsValid(const std::vector<std::string> &patterns) {
    std::regex pattern_f;
    for (auto pattern : patterns) {
      ...
      try {
        ...
        std::string msg =
            fmt::format("Pattern \"{}\" parsing failed: {}", pattern, e.what());
  ...
  static std::string MergeModeStr(const std::string &base_mode_str,
  ...
  static bool IsModeValid(uint64_t mode_int) { return mode_int <= 0777; }
  ...
  //   static bool _match(const std::string &name, const std::string &pattern_f,
...
inline std::vector<std::string> resplit(std::string path, char front_esc,
                                        char back_esc, std::string head = "") {
...
inline std::pair<std::string, PathInfo> stat(const std::string &path,
  ...
  try {
    ...
    return std::make_pair(fmt::format("Stat {} failed: {}", pathf, e.what()),
                          PathInfo());
  }
  ...
  if (info.type == PathType::FILE) {
    try {
      ...
      return std::make_pair(fmt::format("Fail to get file size: {}", e.what()),
                            PathInfo());
    }
  }
...
inline std::vector<PathInfo> iwalk(const std::string &path,
...
// inline std::tuple<std::vector<std::string>, std::string, bool>
// preprocess(std::string path, bool use_regex)
// {
...
//     return std::make_tuple(match_parts, abspath(join(root_parts), false,
//     "\\"), is_recursive);
// }

// inline void search(std::vector<std::string> &results, fs::path root,
// std::string name, std::vector<std::string> remains, SearchType type, bool
// use_regex, bool silence, CB callback)
// {
//     if (!remains.empty())
//     {
...
//                 if (name == "**")
//                 {
//                     next_match = Str::_match(cur_name, remains.front(),
//                     use_regex); if (is_dir)
//                     {
//                         search(results, cur_path, name, remains, type,
//                         use_regex, silence, callback); if (next_match)
//                         {
//                             auto new_name = remains.front();
//                             auto new_remains = remains;
//                             new_remains.erase(new_remains.begin());
//                             search(results, cur_path, new_name, new_remains,
//                             type, use_regex, silence, callback);
//                         }
//                     }
//                     else
//                     {
...
//                 search(results, cur_path, name, remains, type, use_regex,
//                 silence, callback); if (is_match)
//                 {
//                     auto new_name = remains.front();
//                     auto new_remains = remains;
//                     new_remains.erase(new_remains.begin());
//                     search(results, cur_path, new_name, new_remains, type,
//                     use_regex, silence, callback);
//                 }
//                 continue;
//             }
...
//             Str::_match(root.filename().string(), name, use_regex))
//             {
...
//                     continue;
//                 }
//                 search(results, cur_path, "**", {}, type, use_regex, silence,
//                 callback);
//             }
//             return;
//         }
...
// inline std::vector<std::string> find(const std::string &path_f, SearchType
// type = SearchType::All, bool use_regex = false, bool silence = false, CB
// callback = nullptr)
// {
...
//     if (root_path.find("\\") == std::string::npos && root_path.find("/") ==
//     std::string::npos)
//     {
//         root_path = root_path + "\\";
//     }

//     search(results, fs::path(root_path), name_first, match_parts, type,
//     use_regex, silence, callback); return results;
// }
