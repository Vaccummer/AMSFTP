...
public:
  ...
  ClientHandle GetHost(const std::string &nickname) {
...
private:
  ...
  ClientHandle local_client;

  ClientHandle GetHost_(const std::string &nickname) const {
...
public:
  ClientMaintainer(int heartbeat_interval_s = 60,
    ...
    if (local_client && local_client->GetProtocol() == ClientProtocol::LOCAL) {
      ...
      this->local_client = std::make_shared<AMLocalClient>(
          ConRequest{"local", "localhost", "", 64 * AMMB, ClientProtocol::LOCAL,
                     22, "local"});
    }
  ...
  GetHostsSnapshot() {
    ...
    for (const auto &host : hosts) {
      snapshot[host.first] = host.second;
    }
    return snapshot;
  }

  ClientHandle GetClient(const std::string &nickname) {
    std::lock_guard<std::recursive_mutex> lock(beat_mtx);
    if (nickname.empty() || AMStr::lowercase(nickname) == "local") {
      return local_client;
    }
  ...
  std::vector<ClientHandle> GetClients() {
    std::vector<ClientHandle> client_list{};
    std::lock_guard<std::recursive_mutex> lock(beat_mtx);
    if (local_client) {
  ...
  void add_client(const std::string &nickname,
    ...
    hosts[nickname] = std::move(client);
  }
...
private:
  ...
  ResolveBaseClient_(const std::shared_ptr<ClientMaintainer> &hostm,
  ...
  TestHost(const TransferTask &task,
    ...
    if (!task.src_host.empty()) {
      src_client = ResolveBaseClient_(hostm, task.src_host);
      if (!src_client) {
      ...
      src_client = ResolveBaseClient_(hostm, "");
    }
    if (!task.dst_host.empty()) {
      dst_client = ResolveBaseClient_(hostm, task.dst_host);
      if (!dst_client) {
        return {
            ECM{EC::NoSession,
                AMStr::fmt("Destination host \"{}\" not found", task.dst_host)},
            nullptr, nullptr};
      }
      ...
      dst_client = ResolveBaseClient_(hostm, "");
    }
...
public:
   ...
   */
  std::shared_ptr<ClientMaintainer> take_task_host(const TaskId &id) {
  ...
  load_tasks(const std::string &src, const std::string &dst,
    ...
    auto resolve_ready_client = [&](const std::string &nickname)
      ...
      if (rcm.first != EC::Success) {
        rcm = client->Check(interrupt_flag, timeout_ms, start_time);
      }
      return {rcm, client};
    };

    auto [rc1, src_client] = resolve_ready_client(src_host);

    if (rc1.first != EC::Success) {
      return {rc1, tasks};
    }
    auto [rc2, dst_client] = resolve_ready_client(dst_host);
